...
function bind(){
  ...
  if(byId('create-contract'))byId('create-contract').onclick=function(){var button=this;button.disabled=true;setStatus('contract-status','Creating member…');api('/api/bison/members',{name:byId('member-name').value,phone:byId('member-phone').value}).then(function(m){setStatus('contract-status','Creating contract…');return api('/api/bison/contracts',{memberId:m.member.id,studioId:byId('contract-studio').value,nestId:byId('contract-nest').value,monthlyRent:Number(byId('contract-rent').value),deposit:Number(byId('contract-deposit').value||0),startDate:byId('contract-start').value,endDate:byId('contract-end').value,signedStatus:byId('contract-signed').value});}).then(function(c){setStatus('contract-status','Created '+c.contract.id+'.');return load();}).catch(function(e){setStatus('contract-status',e.message,true);}).finally(function(){button.disabled=false;});};
