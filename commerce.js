...
async function api(path,body,method='POST',key) {
  ...
  if(!response.ok){if(response.status===401&&!optionalSetupExpired(path,result.error,cat,account)){if(account||owner.active)sessionExpired=true;account=null;orders=[];nestOrders=[];applications=[];}throw {message:errorText(result.error),code:result.error,status:response.status,result};} return result;
}
...
function applyCommitmentGate(root=document){
  ...
  const copy=shopCopy!==undefined?shopCopy:(saveCommitmentReady()||shoppingOpen)?(['live','earn'].includes(page)||category==='insurance'?OTHER_COMMITMENTS_PAUSED_COPY:!account&&shoppingOpen?'Sign in to see essentials for your location and reserve.':null):PILOT_CLOSED_COPY;
...
function showBuy(id){const p=cat?.products?.find(p=>p.id===id);if(!p||(!cat?.preview&&(p.test===true||p.preview===true)))return;emitAnalytics('save','mill_view');return show(title(p),`<div class="buy-sheet" data-buy-id="${esc(p.id)}"><div class="shop-item-photo">${shopPhoto(p,{esc,t,name:title(p),sourceOwner:cat?.owner})}</div>${p.test?`<p class="info">${t('TEST pack. Pay when you collect.','परीक्षण पैक। लेते समय पैसे दें।')}</p>`:''}<div class="shop-item-detail">${shopPrice(p,{t,esc,money,sourceOwner:cat?.owner})}</div><p class="pack">${esc(t(p.pack||PENDING_PACK))}${cat?.owner==='niasave'?' · '+esc((cat.locations||[]).filter(l=>p.locationIds?.includes(l.id)).map(l=>l.name).join(', ')):''}</p><div class="buy-row">${cart[p.id]&&reserveReady(p)?stepper(p):reserveReady(p)?`<button class="add" data-action="add" data-id="${esc(p.id)}">${t('Add','जोड़ें')}</button>`:`<button class="add" type="button" disabled>${t('Ask Nia','निया से पूछें')}</button>`}</div><p>${cat?.test?t('TEST only. No payment or physical collection.'):t('Pay when you collect.','लेते समय पैसे दें।')}</p></div>`);}
...
function showPhoneLogin(){challenge='';passwordToken='';pendingPassword='';authStep='phone';show(t('Sign in to Niasave','नियासेव में साइन इन करें'),phoneFormMarkup({t,esc,phone:authPhone,passwordLink:usesPasswordAuth(cat),otpPrimary:otpIsPrimary(cat)})+entryLanguage());}
...
if(action==='optional-password'){if(!optionalPasswordAllowed(cat,account))return;return show(t('Set or change password'),setPasswordFormMarkup({t,optional:true})+entryLanguage());}
...
if(action==='fulfillment'){if(pending||busy||!['pickup','delivery'].includes(id)||!saveLocations(cat,account,id,cart).length)return;fulfillment=id;if(!cat.locations.some(l=>l.id===locationId&&l.modes.includes(id)))locationId='';if($('#review-form'))return review();render();document.querySelector(`[data-action="fulfillment"][data-id="${id}"]`)?.focus({preventScroll:true});return;}
...
if(form.id==='login-form'){
  ...
  show(t('Enter your code','अपना कोड डालें'),verifyFormMarkup({t,esc,phone:authPhone,otpPrimary:otpIsPrimary(cat)})+entryLanguage());
}
...
if(form.id==='set-password-form'){
  if(form.dataset.authStep==='optional-password'){
    try {
      const result=await submitOptionalPassword(api,fields,cat,account);
      if(result?.account?.role!=='member')throw {message:t('Password could not be confirmed. Refresh your account before retrying.')};
      form.reset();
      await finishMemberSession();
      toast(t('Password saved. WhatsApp sign-in is still available.'));
    } catch(e) {
      if(e.code==='setup_expired')e.message=t('Verify again by WhatsApp to set or change an optional password.');
      else if(!e.message)e.message=errorText(e.code);
      throw e;
    }
    return;
  }
