...
import {SHOP_CATEGORY_IDS,voiceLanguage,shopCatalogueMarkup,shopViewState,shopPhoto,shopPrice} from './commerce-shop-v2.js';
...
function shop(){if(category==='insurance')return insuranceView();return `<div class="save-shop-canvas shop-v2-canvas">${!owner.active&&pending?.accountId===account?.id?moneyStatusMarkup({status:'requested',offlineQueued:!navigator.onLine,kind:'shop'},{t,esc}):''}${shopCatalogueMarkup({catalogue:cat,query:search,aisle,lang,state:shopViewState({catalogue:cat,online:navigator.onLine,loading:entryLoading,readFailed:shellReadFailed})},{t,esc,money,bag:bag()})}${footer()}</div>`;}
function showBuy(id){const p=cat?.products?.find(p=>p.id===id);if(!p)return;emitAnalytics('save','mill_view');return show(title(p),`<div class="buy-sheet" data-buy-id="${esc(p.id)}"><div class="shop-item-photo">${shopPhoto(p,{esc,t,name:title(p)})}</div>${p.test?`<p class="info">${t('TEST pack. Pay when you collect.','परीक्षण पैक। लेते समय पैसे दें।')}</p>`:''}<div class="shop-item-detail">${shopPrice(p,{t,esc,money})}</div><p class="pack">${esc(t(p.pack||PENDING_PACK))}</p><div class="buy-row">${cart[p.id]&&reserveReady(p)?stepper(p):reserveReady(p)?`<button class="add" data-action="add" data-id="${esc(p.id)}">${t('Add','जोड़ें')}</button>`:`<button class="add" type="button" disabled>${t('Ask Nia','निया से पूछें')}</button>`}</div><p>${t('Pay when you collect.','लेते समय पैसे दें।')}</p></div>`);}
...
function render(){const dispose=disposeEarnMap;disposeEarnMap=()=>{};dispose();if(page==='bag'){bagOpen=true;page='shop';if(location.hash!=='#shop')history.replaceState(null,'','#shop');}const pickingLang=!languageSticky()&&!owner.active;document.body.classList.toggle('mesha-lang-open',pickingLang);document.body.classList.toggle('mesha-dark',!pickingLang);document.body.classList.toggle('mesha-bag-open',bagOpen&&page==='shop');nav();notice();renderShell();document.body.classList.remove('signin-open');if(pickingLang){$('#content').innerHTML=languageCard();prepareJourney(document);return;}if(!cat){$('#content').innerHTML=page==='shop'?shop():entryHomepage();prepareJourney(document);return;}$('#content').innerHTML=page==='home'?entryHomepage():page==='live'?liveView():page==='earn'?earnView():page==='send'?sendView():page==='shop'?shop():page==='orders'?ordersView():accountView();ownerControls();applyCommitmentGate();prepareJourney(document);if(page==='earn'&&!owner.active&&account)disposeEarnMap=mountMap(mapModel(earnData),t);}
