...
import { PENDING_PACK, packReady, reserveReady, commitmentReady, bagHasUnconfirmedPack, stayErrorKind } from './commerce-truth.js';
...
function applyCommitmentGate(root=document){
  ...
  for(const form of root.querySelectorAll('#review-form,#earn-form'))for(const control of form.querySelectorAll('button,input,select'))control.disabled=true;
  const target=root===document?$('#content'):root;
  if(target&&!target.querySelector('[data-pilot-paused]')){const note=document.createElement('p');note.className='info';note.dataset.pilotPaused='';note.setAttribute('role','status');note.textContent=t(PILOT_CLOSED_COPY);target.prepend(note);}
}
...
function stayPanel(){
  ...
  if(!navigator.onLine)return pictureState('network',t("We couldn’t load places. Please try again.",'रहने की जगह लोड नहीं हुई। फिर कोशिश करें।'),`<button class="primary" data-action="live">${t('Try again','फिर कोशिश करें')}</button>`);
...
function bagChip(){if(!count()||bagOpen)return '';return `<button type="button" class="mesha-bag-chip" data-action="open-bag" aria-label="${t('Bag','बैग')} · ${count()} · ${money(total())}"><span>${t('Bag','बैग')} · ${count()} · ${money(total())}</span><span>${t('View','देखें')}</span></button>`;}
function bagLine(p){return `<div class="mesha-bag-line"><button type="button" class="mesha-bag-thumb" data-action="detail" data-id="${p.id}" aria-label="${esc(title(p))}">${photo(p)}</button><div class="mesha-bag-copy"><strong>${esc(title(p))}</strong><small>${t('You keep','आपके पास')} ${money(productKeep(p))}</small>${stepper(p)}</div><strong class="mesha-bag-price">${money(p.price*cart[p.id])}</strong></div>`;}
function bag(){if(owner.active)return '<section class="mesha-bag stack"><div class="mesha-bag-head"><h2>Owner view</h2><button type="button" class="icon-button" data-action="close-bag" aria-label="Close">×</button></div><p>Browse published essentials and prices. Ordering requires member sign in.</p></section>';const n=count();const heading=n?`${t('Bag','बैग')} · ${n}`:t('Bag','बैग');const body=n?(cat.products||[]).filter(p=>cart[p.id]).map(bagLine).join(''):`<div class="mesha-bag-empty"><h2>${t('Your bag is empty','आपका बैग खाली है')}</h2><p>${t('Add oils or soap from Save. Browse free.','Save से तेल या साबुन जोड़ें। मुफ़्त देखें।')}</p></div>`;const foot=n?`<div class="mesha-bag-total"><span>${t('Total','कुल')}</span><strong>${money(total())}</strong></div><p class="mesha-bag-quiet">${t('You keep','आपके पास')} ${money(bagKeepTotal())} ${t('vs kirana','किराना से कम')} · ${t('pay when you collect','लेते समय भुगतान')}</p><button class="primary full mesha-pill mesha-pill-solid" data-action="review" ${!navigator.onLine?'disabled':''}>${t('Continue','आगे बढ़ें')}</button><p class="mesha-bag-note">${t('Phone only on the next step. Nothing to pay online.','फोन अगले कदम पर। ऑनलाइन कुछ नहीं देना है।')}</p>`:`<button class="primary full mesha-pill mesha-pill-solid" data-action="close-bag">${t('Back to Save','Save पर वापस')}</button>`;return `<section class="mesha-bag" aria-label="${esc(heading)}"><div class="mesha-bag-head"><h2>${esc(heading)}</h2><button type="button" class="icon-button" data-action="close-bag" aria-label="${t('Close dialog','बंद करें')}">×</button></div>${body}${foot}</section>`;}
...
function shop(){if(category==='insurance')return insuranceView();const body=aisle?saveMakers():saveAisle();return `<div class="save-shop-canvas"><div class="intro store-head"><h1>${t('Save')}</h1><p>${t('Everyday essentials','रोज़ का सामान')}</p></div>${!aisle&&category==='all'?saveHero():''}${saveSearch()}${saveTabs()}${body}${footer()}</div>${bagChip()}${bagOpen?bagLayer():''}`;}
function showBuy(id){const p=cat?.products?.find(p=>p.id===id);if(!p)return;emitAnalytics('save','mill_view');return show(title(p),`<div class="buy-sheet" data-buy-id="${p.id}">${photo(p,'detail-photo')}${p.test?`<p class="info">${t('TEST pack. Pay when you collect.','परीक्षण पैक। लेते समय पैसे दें।')}</p>`:''}${pricePair(productKeep(p),p.price)}<p class="pack">${esc(t(p.pack||PENDING_PACK))}</p><p>Collect at Nia Nest Ompal</p><div class="buy-row">${cart[p.id]&&reserveReady(p)?stepper(p):reserveReady(p)?`<button class="add" data-action="add" data-id="${p.id}">${t('Add','जोड़ें')}</button>`:`<button class="add" type="button" disabled>${t('Ask Nia','निया से पूछें')}</button>`}</div><p>${t('Pay when you collect.','लेते समय पैसे दें।')}</p>${reserveReady(p)||!commitmentReady()?'':`<p class="info">${t('Ask Nia about this pack.','इस पैक के बारे में निया से पूछें।')}</p>`}${cat.preview?`<p class="info">${t('Illustrative image. Pack size and supplier details need confirmation before the pilot opens.','उदाहरण चित्र। पायलट शुरू होने से पहले पैक साइज़ और सप्लायर जानकारी की पुष्टि चाहिए।')}</p>`:''}</div>`);}
...
function sendView(){if(owner.active)return ownerSendMarkup();const month=booksData?.months?.at(-1);const totals=month?.totals;const sentHome=totals&&Number.isFinite(totals.home)?money(totals.home/100):'—';const signedOut=!account&&!owner.active;const taskAction=signedOut?`<button type="button" class="primary mesha-pill mesha-pill-solid" data-action="login">${t('Sign in','साइन इन')}</button>`:'';return `<section class="store-screen store-send"><header class="store-head store-task"><h1>${t('Send')}</h1><p class="nia-transfer-status" role="status"><span class="badge">${t('Transfers not active','ट्रांसफ़र अभी सक्रिय नहीं')}</span></p><p>${t('This is a money statement and a plan to send home. Saving a plan does not move money.','यह पैसों का ब्यौरा और घर भेजने की योजना है। योजना सहेजने से पैसे नहीं जाते।')}</p>${taskAction}</header><figure class="store-task-photo"><img src="/assets/send.jpg" alt="" width="1168" height="728"></figure>${totals?`<div class="store-price" style="max-width:220px;margin:0 auto 22px"><span class="store-keep"><small>${t('Recorded home amount','घर भेजी दर्ज राशि')}</small><strong>${sentHome}</strong></span></div>`:''}${signedOut?pictureState('signed-out',t('Sign in to see your money plan','पैसे की योजना देखने के लिए साइन इन करें।'),taskAction):booksMarkup(booksData,booksMonth,account,{t,esc,money})}<section class="panel stack books-transfer"><div class="row"><h2>${t('Transfers','ट्रांसफ़र')}</h2><span class="badge">${t('Transfers not active','ट्रांसफ़र अभी सक्रिय नहीं')}</span></div><p>${t('Bank transfers are not available yet. A plan is only an estimate. Saving it does not move money.','बैंक ट्रांसफ़र अभी उपलब्ध नहीं। योजना सिर्फ़ अनुमान है। सहेजने से पैसे नहीं जाते।')}</p></section>${footer()}</section>`;}
...
function nestPhoto(n){
  ...
  const image=media||previewNestImages[n.studioId]||{src:'/assets/studio-bunk-lockers.jpg',width:1024,height:1024};
...
document.addEventListener('click',async event=>{const button=event.target.closest('[data-action]');if(!button)return;event.preventDefault();const action=button.dataset.action,id=button.dataset.id;if(!commitmentReady()&&frozenActions.has(action))return;try{
...
if(action==='shop-hero'){category='food';aisle='';search='';render();return;}
if(action==='open-aisle'){if(!shopTilesFor('all').some(tile=>tile.id===id))return;aisle=id;search='';render();return;}
...
if(form.id==='review-form'&&!commitmentReady())return toast(t(PILOT_CLOSED_COPY));
