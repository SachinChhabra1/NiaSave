...
- `GET /api/commerce/test/place-one` places one unpaid Save order through the same `core.reserve` path as `POST /api/commerce/orders`, persists it to Neon, and returns a signed read URL. A later call returns that order while it is still in flight. Expired, cancelled, returned, missed, or collected TEST orders do not block a new `reserved` order.
