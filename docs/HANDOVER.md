...
- `EARN_SOURCE` — set `central` in Production so signed-in Earn reads Central `earn.projection` and writes `earn.applications` / `earn.apply`. Unset also means Central on the live path (fail closed to unavailable / empty jobs; no invented roles). Isolated preview keeps the local Walk2Work ledger unless this is `central`. Live vacant-nest inventory is not an allowlisted Central member kind; the member Live tab stays honest-empty until founder GO.
