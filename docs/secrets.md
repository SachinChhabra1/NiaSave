...
| STAFF_TOKEN_SECRET | NiaSave Production only; separate value for each hosted nonproduction environment | Human custodians and staff token runtime; every 90 days. Rotation invalidates all existing legacy staff tokens and their signed page cookies. No fallback to SESSION_SECRET or a password. |
...
| MEMBER_PASSWORD | NiaSave Production (member phone) | Shared member sign-in password. Compared with a timing-safe HMAC using SESSION_SECRET. A successful check sets a signed httpOnly session cookie. Rotate on unintended disclosure or cohort change. Does not replace staff passwords or Central service signatures. |
