...
test('Send leads with a planning-only statement and no implied payment', () => {
  ...
  assert.match(sendFn,/Send is a plan\. Saving it does not move money/);
  assert.match(sendFn,/Transfers not active/);
  assert.match(sendFn,/Only a Nia team recorded payment appears as paid/);
  assert.doesNotMatch(sendFn,/sentHome|totals\.home/);
});
...
test('payments stay off while home reads Central member state', () => {
  assert.doesNotMatch(src, /paymentsEnabled\s*=\s*true/);
  assert.match(src, /homeDashboardModel/);
  assert.match(src, /'\/nests\/bookings'/);
  assert.match(src, /'\/earn'/);
