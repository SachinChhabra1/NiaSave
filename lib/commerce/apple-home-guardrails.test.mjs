...
test('AX14 home hero copy is stacked off the photo on phone and inset on desktop', () => {
  ...
  assert.match(apple, /\.nia-home-copy\{/);
  assert.match(apple, /min-height:78vh/);
  assert.match(apple, /border-radius:24px/);
  assert.match(apple, /align-items:stretch/);
});
