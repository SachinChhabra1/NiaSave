...
test('snippet splices into commerce.js and is the only homepage source', () => {
  ...
  assert.doesNotMatch(splice, /desktop must skip the language wall/);
  assert.match(splice, /do not skip desktop/);
});
...
test('homepage is one Live hero plus Earn/Save/Send secondary rail', () => {
  ...
  assert.doesNotMatch(homeFn, /apple-live-unit/);
  assert.doesNotMatch(homeFn, /apple-pair/);
  ...
  assert.doesNotMatch(src, /pickingLang=!languageSticky\(\)&&!owner\.active&&window\.matchMedia/);
  assert.doesNotMatch(splice, /langWide/);
...
test('desktop overlay is full-bleed Apple chrome; phone cage stays under 760', () => {
  ...
  assert.doesNotMatch(apple, /\.apple-save-unit/);
  assert.doesNotMatch(apple, /\.apple-pair/);
