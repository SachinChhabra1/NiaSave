...
test('home keeps its attention cards and LESS tiles in one column at desktop width', () => {
  ...
  assert.match(shellCss,/\.home-attention-grid,\.home-tiles\{grid-template-columns:minmax\(0,1fr\)\}/);
...
test('desktop legacy styling is contained by the centered member shell', () => {
  ...
  assert.match(shellCss, /\.member-shell\{\s*display:block;max-width:440px/);
  assert.match(shellCss, /\.less-nav\{\s*position:fixed;top:auto;left:50%/);
  assert.match(shellCss, /\.header\.header\{\s*box-sizing:border-box;max-width:440px/);
