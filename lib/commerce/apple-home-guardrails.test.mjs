...
test('homepage photos are only existing Live, oil, and committed send assets', () => {
  assert.match(homeFn, /studio-bunk-lockers\.jpg/);
  assert.match(apple, /oils-editorial-sheet\.png/);
  assert.match(apple, /earn-extra-hours-v2\.jpg/);
  assert.match(apple, /send-purpose-family\.jpg/);
...
test('AX14 home hero copy is stacked off the photo; Nest is the product', () => {
