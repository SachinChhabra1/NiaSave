...
test('snippet splices into commerce.js and is the only homepage source', () => {
  ...
  assert.match(splice, /upskilled\/wholesale copy/);
...
test('homepage photos are only existing Live and oil assets', () => {
  ...
  assert.doesNotMatch(apple, /earn-extra-hours-v2\.jpg/);
  assert.doesNotMatch(apple, /send-purpose-family\.jpg/);
  assert.doesNotMatch(apple, /send-village-home\.jpg/);
...
test('AX14 home hero copy is stacked off the photo; bunk is the product', () => {
  ...
  assert.match(apple, /object-position:12% 14%!important/);
  assert.doesNotMatch(apple, /object-position:8% 10%/);
  assert.match(apple, /order:-1/);
  assert.match(apple, /nia-home-hero\.mesha-desire:after\{display:none/);
  assert.match(apple, /background:#ffffff/);
  assert.doesNotMatch(apple, /#0B0B0C/);
  assert.doesNotMatch(apple, /max-width:160px/);
  assert.match(apple, /border-radius:18px/);
  assert.match(homeFn, /nia-home-card-copy/);
  assert.match(homeFn, /nia-home-card-link/);
