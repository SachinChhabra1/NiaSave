...
test('responsive member shell is linked last and copied to dist', () => {
  assert.match(html, /niasave-apple-store\.css/);
  assert.match(html, /theme-color" content="#F5F8FB"/);
  ...
  assert.match(build, /commerce-shell\.css/);
});
...
test('home uses the same Central-backed dashboard at every width', () => {
  assert.match(homeFn, /homeDashboardModel/);
  assert.match(homeFn, /homeDashboardMarkup/);
  assert.doesNotMatch(homeFn, /A Nest near work/);
});

test('primary nav stays LESS and mesha-dark is the single member shell', () => {
  assert.match(src, /\['live','earn','shop','send'\]\.map/);
  assert.match(html, /<nav id="less-nav"/);
  assert.doesNotMatch(html, /id="desktop-nav"|id="mobile-nav"/);
