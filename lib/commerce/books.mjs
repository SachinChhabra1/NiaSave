...
export function ownBooks(s,actor,preview,time){
  ...
  const personal=(b.personal?.[actor.id]||[]).filter(e=>!e.deleted);
  ...
  return mergeBooks(base,[...saveEntries(s,actor,preview),...personal],time);
}
...
export function saveEntries(s,actor,preview){
  ...
  return (s.orders||[]).filter(o=>o.source==='commerce'&&o.memberId===actor.id).flatMap(o=>[
...
export function livingEntries(s,actor,preview){
  ...
  const contracts=new Set((s.bookings||[]).filter(b=>b.source==='member_storefront'&&b.memberId===actor.id).map(b=>b.contractId));
...
export function writePersonalEntry(s,actor,body,key,time){
  ...
  const requestId=actor.id+':'+key,signature=JSON.stringify(body),prior=b.requests[requestId];if(prior){if(prior.signature!==signature)fail('idempotency_conflict',409);return prior.result;}
  const existing=body.id?list.find(e=>e.reference===body.id):null;
