...
export function verifyCentralEnvelope(raw,signature,key,time) {
  ...
  const summaryService=actor?.id==='central-save-summary'&&actor?.role==='service'&&value.line==='save'&&value.action==='save-summary';
  if(!actor||!/^[a-zA-Z0-9_-]{1,100}$/.test(actor.id||'')||(!['admin','operator','reader'].includes(actor.role)&&!summaryService))throw new CommerceError('central_actor_required',403);
