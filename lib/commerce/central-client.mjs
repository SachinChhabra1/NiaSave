...
export const centralKey=(env=process.env)=>{
  if(env.CENTRAL_SERVICE_KEY!==undefined||env.CENTRAL_SERVICE_KEY_ID!==undefined)
    return /^[a-zA-Z0-9_-]{3,80}$/.test(env.CENTRAL_SERVICE_KEY_ID||'')?(env.CENTRAL_SERVICE_KEY||''):'';
  return (env===process.env?showcaseReady():showcaseReady(env))?env.SHOWCASE_CENTRAL_KEY||'':env.CENTRAL_COMMERCE_KEY||'';
};
...
export function centralConfigured(env=process.env){return Boolean(centralOrigin(env.CENTRAL_ORIGIN))&&centralKey(env).length>=32;}
// Server-only, temporary preview transport credential. Never forward a browser
// header, put it in a URL/cookie, or use NiaSave's own automation bypass secret.
export function centralProtectionHeaders(env=process.env){
  const key=env.CENTRAL_PROTECTION_BYPASS_SECRET;
  if(!key)return {};
  const target=centralOrigin(env.CENTRAL_ORIGIN);
  const allowed=centralOrigin(env.CENTRAL_PROTECTION_BYPASS_ORIGIN);
  // Moving to Central's canonical production origin must not leak the preview
  // credential; remove the two temporary settings after the cutover.
  if(target==='https://rafiqicentral.com')return {};
  if(!target||target!==allowed||!new URL(target).hostname.endsWith('.vercel.app')||/[\r\n]/.test(key))
    throw new CommerceError('central_protection_not_configured',503);
...
export async function centralMemberRequest(kind,subject,params={},{fetchImpl=fetch,env=process.env,time=Date.now()}={}){
  ...
  const protectionHeaders={...centralProtectionHeaders(env),...(env.CENTRAL_SERVICE_KEY_ID?{'x-niasave-key-id':env.CENTRAL_SERVICE_KEY_ID}:{})};
  const raw=JSON.stringify(buildEnvelope(kind,subject,params,time));
  let response;
  try{response=await fetchImpl(centralOrigin(env.CENTRAL_ORIGIN)+'/api/service/member',{method:'POST',headers:{'content-type':'application/json',[CENTRAL_SIGNATURE_HEADER]:signCentralRequest(raw,centralKey(env)),...protectionHeaders},body:raw,signal:AbortSignal.timeout(8000),redirect:'error'});}
