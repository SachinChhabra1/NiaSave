...
export async function centralIdentityVerifyConfirm(challenge,code,phone,opts={}){
  ...
  if(result.status===200 && validSubject(account?.id) && account.role==='member' && /^[a-f0-9]{64}$/.test(account.authVersion||'')){
...
export function centralOtpAccount(account,phone){
  const ids=Array.isArray(account.locationIds)?account.locationIds.filter(validSubject):[];
...
export async function refreshCentralOtpAccount(actor,opts={}){
  ...
  if(result.status!==200||body?.source!=='central'||body?.status!=='ready'||m?.id!==actor.id||m?.kyc!=='approved'||m?.access!=='active'||m?.state!=='approved'||a?.id!==actor.id||a?.role!=='member'||a?.authVersion!==actor.authVersion)throw new CommerceError('sign_in_required',401);
...
export async function centralMemberRequest(kind,subject,params={},{fetchImpl=fetch,env=process.env,time=Date.now()}={}){
  ...
  let response;
  try{response=await fetchImpl(env.CENTRAL_ORIGIN.replace(/\/$/,'')+'/api/service/member',{method:'POST',headers:{'content-type':'application/json',[CENTRAL_SIGNATURE_HEADER]:signCentralRequest(raw,centralKey(env))},body:raw,signal:AbortSignal.timeout(kind==='identity.verify.request'?30000:8000),redirect:'error'});}
