...
export async function centralIdentityVerifyConfirm(challenge,code,phone,opts={}){
  ...
  if(result.status===200 && validSubject(account?.id) && account.role==='member'){
    return {accountId:account.id,account:{id:account.id,authSubject:account.id,role:'member',name:typeof account.name==='string'?account.name.slice(0,120):'Nia member',locationIds:Array.isArray(account.locationIds)?account.locationIds.filter(id=>typeof id==='string'):[],phoneVerified:true,phone}};
  }
  if(result.status===429)throw new CommerceError('too_many_attempts',429);
