...
const isShowcase=env=>env===process.env?showcaseReady():showcaseReady(env);
export const centralKey=(env=process.env)=>isShowcase(env)?env.SHOWCASE_CENTRAL_KEY||'':env.CENTRAL_COMMERCE_KEY||'';
...
export async function centralMemberRequest(kind,subject,params={},{fetchImpl=fetch,env=process.env,time=Date.now()}={}){
  ...
  // A server-only integration credential for the configured Central preview.
  // Never send it to a custom/external host or from the ordinary member runtime.
  const protectionHeaders=isShowcase(env)&&new URL(env.CENTRAL_ORIGIN).hostname.endsWith('.vercel.app')&&env.SHOWCASE_CENTRAL_VERCEL_BYPASS
    ? {'x-vercel-protection-bypass':env.SHOWCASE_CENTRAL_VERCEL_BYPASS} : {};
  let response;
  try{response=await fetchImpl(env.CENTRAL_ORIGIN.replace(/\/$/,'')+'/api/service/member',{method:'POST',headers:{...protectionHeaders,'content-type':'application/json',[CENTRAL_SIGNATURE_HEADER]:signCentralRequest(raw,centralKey(env))},body:raw,signal:AbortSignal.timeout(8000),redirect:'error'});}
...
export async function earnProjectionFromCentral(actor,opts={}){
  ...
  try{
    const result=await centralMemberRequest('earn.projection',actor.id,{},opts);
