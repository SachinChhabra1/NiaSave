...
export async function centralIdentityVerifyRequest(phone,opts={}){
  if(!/^\+91[6-9]\d{9}$/.test(phone||''))throw new CommerceError('invalid_phone');
  const result = await centralMemberRequest('identity.verify.request',PHONE_LOOKUP_SUBJECT,{phone,channel:'whatsapp'},opts);
...
export async function centralIdentityVerifyConfirm(challenge,code,opts={}){
  if(typeof challenge!=='string'||!challenge)throw new CommerceError('bad_otp',401);
  if(!/^\d{4,8}$/.test(code||''))throw new CommerceError('bad_otp',401);
  const result = await centralMemberRequest('identity.verify.confirm',PHONE_LOOKUP_SUBJECT,{challenge,code},opts);
  const id = result.body?.account?.id;
  if(result.status===200 && validSubject(id)){
    return {accountId:id,token:typeof result.body?.token==='string'?result.body.token:null};
  }
