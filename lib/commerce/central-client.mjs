...
export function centralConfigured(env=process.env){return /^https:\/\//.test(env.CENTRAL_ORIGIN||'')&&(env.CENTRAL_COMMERCE_KEY||'').length>=32;}
...
export async function centralMemberRequest(kind,subject,params={},{fetchImpl=fetch,env=process.env,time=Date.now()}={}){
  ...
  let response;
  try{response=await fetchImpl(env.CENTRAL_ORIGIN.replace(/\/$/,'')+'/api/service/member',{method:'POST',headers:{'content-type':'application/json',[CENTRAL_SIGNATURE_HEADER]:signCentralRequest(raw,env.CENTRAL_COMMERCE_KEY)},body:raw,signal:AbortSignal.timeout(8000),redirect:'error'});}
