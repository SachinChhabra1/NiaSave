...
// Preview keeps the local Walk2Work ledger unless EARN_SOURCE=central.
// Production treats unset as central so a missing env never invents open roles.
export function earnUsesCentral(preview,env=process.env){
  if(String(env.EARN_SOURCE||'').trim().toLowerCase()==='central')return true;
  return preview!==true;
}
...
export async function earnProjectionFromCentral(actor,opts={}){
  const unavailable=reason=>({preview:false,owner:'Walk2Work',map:{status:'unavailable',asOf:null,revision:null,reason},jobs:[],source:'central'});
  ...
  try{
    ...
    if(result.status!==200||!projection?.map||!Array.isArray(projection.jobs))return unavailable(result.body?.error||'central_unavailable');
    const status=['ready','studio_missing','unavailable','stale'].includes(projection.map.status)?projection.map.status:'unavailable';
    return {
      preview:false,
      owner:projection.owner||'Walk2Work',
      map:{...projection.map,status},
      jobs:status==='ready'?projection.jobs:[],
      member:result.body.member,
      source:'central'
    };
