...
const {buildEnvelope,centralConfigured,centralMemberRequest,centralMemberPhoneLookup,centralIdentityVerifyRequest,centralIdentityVerifyConfirm,planWriteRequest,signCentralRequest,CENTRAL_SIGNING_CONTEXT}=await import('./central-client.mjs');
...
test('identity verify goes to Central kinds, not COMMERCE_IDENTITY_URL', async () => {
  ...
  const fetchImpl=async(url,init)=>{
    ...
    if(kind==='identity.verify.confirm')return {status:200,json:async()=>({token:'sess',account:{id:'nm-kyc-1'}})};
    return {status:400,json:async()=>({error:'unknown_request'})};
  };
  const requested=await centralIdentityVerifyRequest('+919876543210',{env,fetchImpl});
  ...
  const confirmed=await centralIdentityVerifyConfirm('opaque-challenge','123456',{env,fetchImpl});
  assert.equal(confirmed.accountId,'nm-kyc-1');
  await assert.rejects(centralIdentityVerifyRequest('+919876543210',{env,fetchImpl:async()=>({status:400,json:async()=>({error:'unknown_request'})})}),/otp_unavailable/);
  await assert.rejects(centralIdentityVerifyConfirm('opaque-challenge','000000',{env,fetchImpl:async()=>({status:200,json:async()=>({ok:true})})}),/bad_otp/);
});
