...
test('production Earn defaults to Central; preview keeps the local ledger unless EARN_SOURCE=central',async()=>{
  const {earnUsesCentral}=await import('./central-client.mjs');
  assert.equal(earnUsesCentral(true,{}),false);
  assert.equal(earnUsesCentral(true,{EARN_SOURCE:'central'}),true);
  assert.equal(earnUsesCentral(false,{}),true);
  assert.equal(earnUsesCentral(false,{EARN_SOURCE:''}),true);
  assert.equal(earnUsesCentral(false,{EARN_SOURCE:'central'}),true);
});
...
test('Earn from Central passes the projection through and turns every failure into an explicit unavailable map',async()=>{
  ...
  const invented=await earnProjectionFromCentral({id:'member-42',role:'member'},{env,fetchImpl:async()=>({status:200,json:async()=>({projection:{owner:'Walk2Work',map:{status:'unavailable'},jobs:[{id:'invented'}]}})})});
  assert.equal(invented.map.status,'unavailable');assert.deepEqual(invented.jobs,[]);assert.equal(invented.source,'central');
});
