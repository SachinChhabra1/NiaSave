...
test('Central preview protection credential stays server-side, showcase-only, and on the configured Vercel origin',async()=>{
  const env={NIA_SHOWCASE:'1',NIA_SHOWCASE_ENTRY:'isolated-v1',VERCEL_ENV:'preview',SHOWCASE_INSTANCE:'showcase-abcdefgh',SHOWCASE_DATABASE_URL:'postgres://x',SHOWCASE_PASSWORD:'p'.repeat(24),SHOWCASE_CENTRAL_KEY:'s'.repeat(40),CENTRAL_COMMERCE_KEY:KEY,CENTRAL_ORIGIN:'https://central-test.vercel.app',SHOWCASE_CENTRAL_VERCEL_BYPASS:'test-protection-credential'};
  let captured;
  const fetchImpl=async(url,init)=>{captured={url,...init};return Response.json({month:'2026-09',revision:0});};
  await centralMemberRequest('plan.read','preview-member',{}, {env,fetchImpl});
  assert.equal(captured.url,env.CENTRAL_ORIGIN+'/api/service/member');
  assert.equal(captured.headers['x-vercel-protection-bypass'],env.SHOWCASE_CENTRAL_VERCEL_BYPASS);
  assert.equal(captured.redirect,'error');
  assert.ok(!captured.body.includes(env.SHOWCASE_CENTRAL_VERCEL_BYPASS));
  await centralMemberRequest('plan.read','member-42',{}, {env:{...env,NIA_SHOWCASE:'0'},fetchImpl});
  assert.equal(captured.headers['x-vercel-protection-bypass'],undefined);
  await centralMemberRequest('plan.read','preview-member',{}, {env:{...env,CENTRAL_ORIGIN:'https://central.example.com'},fetchImpl});
  assert.equal(captured.headers['x-vercel-protection-bypass'],undefined);
});
