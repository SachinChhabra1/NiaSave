...
test('identity verify goes to Central kinds, not COMMERCE_IDENTITY_URL', async () => {
  ...
  const fetchImpl=async(url,init)=>{
    ...
    if(kind==='identity.verify.confirm')return {status:200,json:async()=>({token:'sess',account:{id:'nm-kyc-1',role:'member'}})};
...
test('OTP confirmation rejects missing identity, wrong role and masked upstream 401; outages stay retryable',async()=>{
 ...
 const seen=[];
 const confirmed=await centralIdentityVerifyConfirm('challenge','123456','+919876543210',{env,fetchImpl:async(_url,init)=>{seen.push(JSON.parse(init.body));return {status:200,json:async()=>({account:{id:'canonical-member',authSubject:'ignored-subject',role:'member',locationIds:[]},token:'never-exposed'})};}});
