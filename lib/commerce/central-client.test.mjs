...
test("M0 refuses Earn submission even when Central Earn is configured",async()=>{process.env.EARN_SOURCE='central';try{const r=await request('/earn/applications',{jobId:'fixture',revision:'1',consent:true});assert.equal(r.status,503);assert.equal(r.body.error,'pilot_commitments_paused');}finally{delete process.env.EARN_SOURCE;}});
