...
test('GET /test/identity is credential-free, labelled TEST, and does not call Central', async () => {
  const originalFetch = globalThis.fetch;
  let called = false;
  globalThis.fetch = async () => { called = true; throw new Error('central_must_not_run'); };
  try {
    const proof = await request('/test/identity');
    assert.equal(proof.status, 200, proof.body?.error);
    assert.equal(proof.body.test, true);
    assert.equal(proof.body.source, 'test');
    assert.equal(proof.body.member.id, TEST_PHONE_MEMBER.id);
    assert.equal(proof.body.member.name, 'TEST member');
    assert.equal(proof.body.paymentsEnabled, false);
    assert.equal(proof.body.copiedToNeon, false);
