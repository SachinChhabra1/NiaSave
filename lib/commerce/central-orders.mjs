...
export function presentCentralOrder(row){
  ...
  if(row.payment != null){
    const p=row.payment;
    if(row.effectiveStatus!=='completed'||p.status!=='received'||p.method!=='at_pickup'||
      p.bankSettlementVerified!==false||p.amountPaise!==row.totalPaise||
      !Number.isFinite(Date.parse(p.receivedAt)))fail();
    payment={status:'received',method:'at_pickup',amountPaise:p.amountPaise,
      receivedAt:p.receivedAt,bankSettlementVerified:false};
  }
