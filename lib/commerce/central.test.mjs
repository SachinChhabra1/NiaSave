...
test('NiaBooks reads the Central projection through the member session; score consent and demo publication are gated',async()=>{
  ...
  const member=await login();const result=await http('/api/commerce/books',undefined,member);assert.equal(result.status,200);assert.equal(result.body.months.length,3);
