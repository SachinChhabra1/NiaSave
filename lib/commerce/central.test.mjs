...
test('Central signature binds actor/body, rejects missing keys, tampering, old requests and replay',async()=>{
  ...
  const envelope={actor:admin,line:'earn',action:'snapshot',at:Date.now(),nonce:randomUUID()};const raw=JSON.stringify(envelope),sig=centralSignature(raw,process.env.CENTRAL_COMMERCE_KEY);
...
test('Save snapshot refuses a local demo instead of presenting seeded stock as NiaSave source',async()=>{
  const result=await central('save','snapshot');
  assert.equal(result.status,503);assert.equal(result.body.error,'durable_operations_required');
});
