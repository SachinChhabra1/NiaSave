...
test('Central Save reads and changes the exact member order, with reader/location/finance restrictions',async()=>{
  const s=fresh();const member=await login();confirmPreviewPacks(s);const intent={locationId:'S01',fulfillment:'pickup',lines:[{id:'groundnut_oil',qty:1}]};
...
test('Connected demo requires local preview, scopes the residence and preserves closed jobs on reload',async()=>{
 fresh();const member=await login();const input={memberId:'preview-member',studio:{id:'demo-hsr',name:'Demo Nest',lat:12.91,lng:77.64,verified:true},jobs:Array.from({length:4},(_,i)=>({...offer(),source:{...offer().source,id:'DEMO-W2W-'+i,workplace:{lat:12.92+i/100,lng:77.65,verified:true},mandateStatus:'open',openPositions:2}}))};
 assert.equal((await central('earn','demo',input,operator)).status,403);
 assert.equal((await central('earn','demo',input)).status,200);
 assert.equal((await http('/api/commerce/earn')).body.map.status,'unavailable');
 const own=await http('/api/commerce/earn',undefined,member);assert.equal(own.body.map.status,'ready');assert.equal(own.body.map.studio.name,'Demo Nest');assert.equal(own.body.jobs.length,4);
 const first=own.body.jobs[0];await central('earn','action',{action:'unpublish',jobId:first.id});await central('earn','demo',input);
 assert.equal((await http('/api/commerce/earn',undefined,member)).body.jobs.length,3);
 const {seedDemo}=await import('./earn.mjs');assert.throws(()=>seedDemo({},admin,input,false,Date.now()),/local_demo_only/);
 process.env.NODE_ENV='production';try{assert.equal((await central('earn','demo',input)).status,503);}finally{process.env.NODE_ENV='test';}
});
test('NiaBooks reads the Central projection through the member session; score consent and demo publication are gated',async()=>{
  fresh();const now=new Date(),body={memberId:'preview-member',months:[3,2,1].map(i=>({month:new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth()-i,1)).toISOString().slice(0,7),complete:true,entries:[]}))};
  assert.equal((await central('send','books-demo',body,operator)).status,403);
  assert.equal((await central('send','books-demo',body)).status,200);
  assert.equal((await http('/api/commerce/books')).status,401);
  const member=await login();const result=await http('/api/commerce/books',undefined,member);assert.equal(result.status,200);assert.equal(result.body.months.length,4);
  assert.equal((await http('/api/commerce/books/consent',{enabled:true},{...member,origin:'https://outside.example'})).status,403);
  assert.equal((await http('/api/commerce/books/consent',{enabled:true},member)).status,200);
  assert.equal((await http('/api/commerce/books',undefined,member)).body.score.reason,'history_incomplete');
  const reader=await central('send','snapshot',{}, {id:'reader',role:'reader'});assert.equal(reader.body.books.demoMembers,0);
  assert.equal((await central('send','action',{})).body.error,'send_not_enabled');
});
