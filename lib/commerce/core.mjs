...
function stock(s, id) {
  return Math.max(0, (s.beat.opening[id] || 0) - s.orders.filter(o => o.beatDate === s.beat.beatDate && !['cancelled','expired','returned','missed'].includes(o.status)).reduce((sum,o) => sum + o.lines.filter(l => l.id === id).reduce((n,l) => n + l.qty, 0), 0));
}
export function catalogue(s, skus, preview, time) {
  ...
  return {
    ...
    products: ready ? config.products.map(p => ({ ...p, price: skus.find(k => k.id === p.id)?.nia, available: stock(s,p.id) })) : [],
  };
}
...
function cleanLines(body, products) {
  ...
  return body.lines.map(l => {
    ...
    return { id:p.id, name:p.name, pack:p.pack, qty:l.qty, nia:p.price, kirana:p.price };
...
export function quote(s, actor, body, skus, preview, time) {
  ...
  const lines = cleanLines(body,cat.products);
  const amount = lines.reduce((n,l) => n + l.nia*l.qty,0);
  ...
  return { lines, amount, location, fulfillment:body.fulfillment, fingerprint, expiresAt:iso(Math.min(Math.max(time,Date.parse(location.windowStart))+cat.reserveMinutes*60000,Date.parse(location.windowEnd))) };
}
...
export function configure(s,skus,body,actor,time) {
  ...
  const products = body.products.map(p => {
    if (!skus.some(s => s.id === p.id) || !p.name?.trim() || !p.pack?.trim() || !['Cooking','Home care','Personal care'].includes(p.category)) fail('verified_product_details_required');
    return { id:p.id,name:String(p.name).slice(0,80),hindi:String(p.hindi||'').slice(0,100),pack:String(p.pack).slice(0,100),category:p.category,illustration:descriptions[p.id]?.[3] || 'bag' };
  });
  const locations = body.locations.map(l => {
    ...
    return {id:l.id,name:String(l.name).slice(0,80),address:String(l.address).slice(0,250),modes:[...new Set(l.modes)],windowStart:l.windowStart,windowEnd:l.windowEnd};
  });
