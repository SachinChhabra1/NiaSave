...
export function catalogue(s, skus, preview, time) {
  ...
  return {
    ...
    products: ready ? config.products.map(p => ({ ...p, category:goodsCategory(p.category)||p.category, price: skus.find(k => k.id === p.id)?.nia, available: stock(s,p.id) })) : [],
  };
}
...
export function configure(s,skus,body,actor,time) {
  ...
  const products = body.products.map(p => {
    ...
    return { id:p.id,name:String(p.name).slice(0,80),hindi:String(p.hindi||'').slice(0,100),pack:String(p.pack).slice(0,100),category:goodsCategory(p.category),illustration:descriptions[p.id]?.[3] || 'bag' };
  });
