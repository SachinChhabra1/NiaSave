...
import { applyTestLane, isTestFlagged, TEST_DESCRIPTION, TEST_SKU_ID } from './test-lane.mjs';
...
export function initialise(s, skus, preview, time) {
  ...
  if (preview && !c.config) {
    c.config = {
      ...
      locations: [{ id: 'S01', name: 'Nia Nest Ompal', address: 'Ved Road · Sukh Store · Theatre North · TEST collection point', modes: ['pickup', 'delivery'], windowStart: iso(time), windowEnd: iso(time + 14 * 86400000) }],
      products: skus.filter(p => descriptions[p.id]).map(p => ({
        id: p.id,
        name: descriptions[p.id][0],
        hindi: descriptions[p.id][1],
        category: descriptions[p.id][2],
        illustration: descriptions[p.id][3],
        pack: isTestFlagged(p) ? (p.pack || '1 L bottle') : 'Pack size to be confirmed',
        ...(isTestFlagged(p) ? { test: true, testFlagged: true, price: p.nia || p.price || 185, keep: p.keep || 75, kirana: p.kirana || 255 } : {})
      })),
      staffLocations: {},
      support: 'Visit your Nia pickup team for help.'
    };
    applyTestLane(s, time, skus);
  }
  return c;
}
...
export function publicBrowseProducts(skus) {
  return (skus || []).filter(p => descriptions[p.id] && !isTestFlagged(p)).map(p => {
...
export function catalogue(s, skus, preview, time, actor = null) {
  ...
  const signedInMember = actor?.role === 'member';
  return {
    ...
    products: ready ? config.products.filter(p => !isTestFlagged(p) || signedInMember).map(p => ({ ...p, category:goodsCategory(p.category)||p.category, price: p.pricePaise!==undefined?p.pricePaise/100:skus.find(k => k.id === p.id)?.nia ?? p.price, available: freshProduct(p,time)?stock(s,p.id):0, opening:s.beat.opening[p.id]||0 })) : [],
  };
}
...
export function publicOrder(o) {
  return Object.fromEntries(['id','pickupCode','lines','amount','due','paid','payStatus','status','createdAt','updatedAt','expiresAt','location','fulfillment','payment','refund','preview','test'].map(k => [k,o[k]]));
}
...
function cleanLines(body, products) {
  ...
  return body.lines.map(l => {
    ...
    return { id:p.id, name:p.name, pack:p.pack, qty:l.qty, nia:p.price, kirana:p.price, ...(isTestFlagged(p)?{test:true}:{}), ...(p.sourceSku?{sourceSku:p.sourceSku,sourceSiteCode:p.sourceSiteCode}:{}) };
  }).sort((a,b) => a.id.localeCompare(b.id));
}
export function quote(s, actor, body, skus, preview, time) {
  member(actor);
  const cat = catalogue(s,skus,preview,time,actor);
...
export function reserve(s, actor, body, key, skus, preview, time) {
  ...
  const o = { id:'ord-'+randomUUID(), source:'commerce', memberId:actor.id, member:actor.name,
    ...
    test: q.lines.some(l => isTestFlagged(l) || l.id === TEST_SKU_ID) };
