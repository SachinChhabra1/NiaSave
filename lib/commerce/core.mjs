...
/** QC/PRD names. Legacy at_stop/collected still accepted as aliases. */
export const ORDER_STATUS = Object.freeze({
  ...
  collect_at_stop: 'collect_at_stop'
});
const STATUS_ALIAS = Object.freeze({
  at_stop: 'out_for_delivery',
  arrived: 'out_for_delivery',
  'out-for-delivery': 'out_for_delivery',
  collected: 'collect_at_stop',
  delivered: 'collect_at_stop',
  'collect-at-stop': 'collect_at_stop'
});
export function canonStatus(status) {
  const key = String(status || '');
  return STATUS_ALIAS[key] || key;
}
const active = new Set(['reserved', 'picked', 'packed', 'loaded', 'out_for_delivery', 'at_stop', 'return_pending']);
...
export function expire(s, time) {
  if (COMMITMENTS_FROZEN) return;
  for (const o of s.orders) {
    if (o.source !== 'commerce' || !active.has(o.status) || Date.parse(o.expiresAt) > time || o.status === 'return_pending') continue;
    o.status = ['reserved', 'picked'].includes(canonStatus(o.status)) ? 'expired' : 'return_pending';
...
export function publicBrowseProducts(skus) {
  return (skus || []).filter(p => descriptions[p.id]).map(p => {
    const [name, hindi, category] = descriptions[p.id];
    return {
      id: p.id,
      name,
      hindi,
      category: goodsCategory(category) || category,
      pack: 'Pack size to be confirmed',
      price: p.nia,
      keep: p.keep,
      kirana: p.kirana,
      available: Number.isFinite(p.opening) ? p.opening : 8
    };
  });
}
export function catalogue(s, skus, preview, time) {
  ...
  return {
    ...
    locations: ready ? config.locations.filter(l => (preview || l.test !== true) && Date.parse(l.windowEnd) > time) : [],
    products: ready ? config.products.filter(p => preview || p.test !== true).map(p => ({ ...p, category:goodsCategory(p.category)||p.category, price: p.pricePaise!==undefined?p.pricePaise/100:skus.find(k => k.id === p.id)?.nia, available: freshProduct(p,time)?stock(s,p.id):0, opening:s.beat.opening[p.id]||0 })) : [],
  };
}
...
function orderFor(s, actor, id, staff = false) {
  ...
  if (!o || (!staff && !ownsMemberRecord(actor,o.memberId))) fail('order_not_found',404);
  return o;
}
export function publicOrder(o) {
  const row = Object.fromEntries(['id','memberId','pickupCode','lines','amount','due','paid','payStatus','status','createdAt','updatedAt','expiresAt','location','fulfillment','payment','refund','preview','test'].map(k => [k,o[k]]));
  row.status = canonStatus(row.status);
  return row;
}
export function listOrders(s, actor) { member(actor); return s.orders.filter(o => o.source === 'commerce' && ownsMemberRecord(actor,o.memberId)).map(publicOrder).reverse(); }
function cleanLines(body, products) {
  ...
  return body.lines.map(l => {
    ...
    if (!packIsConfirmed(p)) fail('pack_unconfirmed',409);
    if (p.available < l.qty) fail('stock_changed',409);
    return { id:p.id, name:p.name, pack:p.pack, qty:l.qty, nia:p.price, kirana:p.price, ...(p.test?{test:true}:{}), ...(p.sourceSku?{sourceSku:p.sourceSku,sourceSiteCode:p.sourceSiteCode}:{}) };
...
export function quote(s, actor, body, skus, preview, time) {
  ...
  if(['central-passkey','central-whatsapp'].includes(actor.identitySource)&&!actor.locationModes?.[location.id]?.includes(body.fulfillment))fail('location_unavailable',409);
  if(actor.identitySource==='central-whatsapp'&&body.fulfillment==='delivery'&&!/^[1-9]\d{5}$/.test(actor.locationPinCodes?.[location.id]||''))fail('delivery_location_unverified',409);
  ...
  const lines = cleanLines(body,cat.products.filter(p=>!p.sourceSiteCode || p.sourceSiteCode===location.sourceSiteCode || p.sourceSiteCode===location.id));
...
export function reserve(s, actor, body, key, skus, preview, time) {
  ...
  const prior=memberOwnerIds(actor).map(id=>s.commerce.requests[hash(id+':'+key)]).filter(Boolean);
  if(new Set(prior.map(p=>p.orderId)).size>1)fail('history_reconciliation_required',409);
  ...
  if (s.orders.filter(o => ownsMemberRecord(actor,o.memberId) && active.has(o.status)).length >= 3) fail('too_many_active_orders',409);
  let pickupCode;
  ...
  const o = { id:'ord-'+randomUUID(), source:'commerce', memberId:actor.id, member:actor.name,
    ...
    preview, method:'upi', cart:'studio', owner:'Sikh Unit', kept:0, date:iso(time).slice(0,10), test:q.lines.some(l=>l.test===true) };
...
export function staffAction(s,actor,body,time) {
  ...
  if (body.action === 'verify_payment') {
    ...
    const here = canonStatus(o.status);
    if (!/^\d{12}$/.test(ref) || body.amount !== o.amount || body.receiptVerified !== true) fail('verify_exact_receipt_amount_and_reference');
    if (!['out_for_delivery', 'collect_at_stop'].includes(here)) fail('payment_only_at_handover',409);
    if (o.payment) {
    ...
    o.payment = { reference:ref, amount:o.amount, verifiedBy:actor.id, verifiedAt:iso(time), method:'upi', ...(o.test?{test:true}:{}) };
    o.payStatus = 'paid'; o.paid = o.amount; o.due = 0;
    ...
    if (!o.payment || !['paid','verified'].includes(o.payStatus)) fail('verified_payment_required',409);
    if (!String(body.note || '').trim() || body.statementVerified !== true) fail('statement_verification_required');
    const settlement = s.settlements.find(v => v.orderId === o.id);
    ...
    const action = canonStatus(body.action);
    const here = canonStatus(o.status);
    const transitions = {
      reserved: ['picked', 'packed', 'cancelled'],
      picked: ['packed', 'cancelled'],
      packed: ['loaded', 'return_pending'],
      loaded: ['out_for_delivery', 'return_pending'],
      out_for_delivery: ['collect_at_stop', 'return_pending'],
      collect_at_stop: ['return_pending'],
      return_pending: ['returned']
    };
    if (!transitions[here]?.includes(action)) fail('invalid_order_transition',409);
    if (action === 'collect_at_stop' && (String(body.pickupCode).toUpperCase() !== o.pickupCode || !['paid','verified','reconciled'].includes(o.payStatus))) fail('pickup_code_and_verified_payment_required',409);
    if (action === 'returned' && body.stockInspected !== true) fail('stock_inspection_required');
    if (o.payment && ['cancelled','returned'].includes(action) && !o.refund?.reconciled) fail('paid_return_requires_refund_resolution',409);
    o.status = action;
    s.scans.push({id:'scan-'+randomUUID(),orderId:o.id,type:o.status,actor:actor.id,at:iso(time)});
    if (terminal.has(o.status) || canonStatus(o.status) === 'collect_at_stop') s.reservations = s.reservations.filter(r => r.orderId !== o.id);
  }
  audit(); return publicOrder(o);
}
