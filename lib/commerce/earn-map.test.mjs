...
test('authoritative stale and malformed projections stay distinct from fresh empty results',()=>{
 assert.equal(mapModel({map:{status:'stale',asOf:new Date(now).toISOString()},jobs:[]},now).status,'stale');
 assert.equal(mapModel({map:{status:'ready',asOf:new Date(now).toISOString(),studio:point},jobs:null},now).status,'unavailable');
 assert.equal(mapModel(source([]),now).status,'ready');
 const label=(en,hi)=>en;
 assert.match(mapMarkup({status:'stale'},label,()=>''),/Job locations need refreshing/);
 assert.match(mapMarkup({status:'unavailable'},label,()=>''),/neighbourhood map is being connected/);
});
