...
export function jobs(s, preview, time) {
  return { preview, owner:'Walk2Work', map:{status:'unavailable',reason:'central_member_map_not_connected'}, jobs:earnState(s).jobs.filter(j => j.status === 'published' && Date.parse(j.closesAt) > time && (preview || !j.preview)).map(publicJob) };
}
