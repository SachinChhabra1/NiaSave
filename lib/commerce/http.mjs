...
export async function commerceHttp(req,res,path,getStaff) {
  ...
  if (path === '/books/plan') {
    ...
    try {
      ...
      const result=req.method==='GET'
        ? await central.centralMemberRequest('plan.read',actor.id,month?{month}:{})
        : await central.centralMemberRequest('plan.write',actor.id,central.planWriteRequest(body,req.headers['idempotency-key']));
  ...
  // Earn from Central (EARN_SOURCE=central): read the member's projection server to
  // server, outside any replayable callback. Off by default until PR C moves applications.
  if (req.method==='GET' && path==='/earn' && !preview && process.env.EARN_SOURCE==='central') {
    try {
      const auth=await withSaveState(s=>{core.initialise(s,SKUS,preview,time);if(s.dummy!==false)return {status:503,body:{error:'preview_data_cannot_go_live'}};return {status:200,body:{actor:sessionActor(s,req,time)}};});
      if(auth.status>=400)return send(auth.status,auth.body);
      return send(200,await central.earnProjectionFromCentral(auth.body.actor));
  ...
  if (path.startsWith('/nests')) {
    try {
      ...
      if(!auth.body.allowed)return send(429,{error:'too_many_attempts'});
      const actor=auth.body.actor;
      const result=await withLivingState(s=>{
        if(!preview&&s.dummy!==false)return {status:503,body:{error:'preview_data_cannot_go_live'}};
        ...
        if(req.method==='PUT'&&path==='/nests/config')return out(nests.configureNests(s,body,getStaff(req),time));
    ...
    const result = await withSaveState(async s => {
      ...
      try {
        ...
        if(req.method === 'POST' && ['/auth/verify','/auth/preview'].includes(path)) {
          ...
          return out({account},200,{'set-cookie':`${cookieName}=${token}; Path=/api/commerce; HttpOnly; SameSite=Strict; Max-Age=43200${preview&&!hosted?'':'; Secure'}`});
        }
