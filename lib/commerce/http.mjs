...
export async function commerceHttp(req,res,path,getStaff) {
  ...
  if(!preview && passwordAuthEnabled()){
    ...
    if(req.method==='POST' && path==='/auth/login'){
      ...
      if(result.status===200&&isLegacyPersonalPasswordProfile(profile)){
        let migrated;
        try{
          migrated=buildMemberPasswordProfile(phone,body.password,{subject:profile.subject,locationIds:profile.locationIds,name:profile.name,identitySource:profile.identitySource,authVersion:profile.authVersion,locationModes:profile.locationModes,locationPinCodes:profile.locationPinCodes},time);
  ...
  if(!preview && req.method==='GET' && path==='/catalogue'){
    try{
      ...
      return send(200,{...view,memberAuth:passwordAuthEnabled()?'password':passkeysEnabled()?'passkey':'legacy',
        ...(passwordAuthEnabled()?{memberAuthCapabilities:passwordAuthCapabilities()}:{}),
        account:{...publicMemberAccount(actor),locationIds:view.locations.map(loc=>loc.id)}});
  ...
  if(!preview && req.method==='GET' && path==='/orders/retry-status'){
    try{
      const session=await withSaveRead(s=>({status:200,body:{actor:sessionActor(s,req,time)}}));
      const actor=session.body?.actor;
      if(!actor)return send(401,{error:'sign_in_required'});
      if(actor.role!=='member')return send(403,{error:'member_access_required'});
      const key=new URL(req.url,'https://niasave.invalid').searchParams.get('key');
      const result=await retryReserveStatus(central,actor.authSubject||actor.id,key);
      return send(result.status,result.body);
    }catch(e){return send(e instanceof core.CommerceError?e.status:503,
      {error:e instanceof core.CommerceError?e.message:'service_unavailable'});}
  }
  ...
  if(!preview && req.method==='POST' && path==='/quote'){
    try{
      const session=await withSaveRead(s=>({status:200,body:{actor:sessionActor(s,req,time)}}));
      const actor=session.body?.actor;
      if(!actor)return send(401,{error:'sign_in_required'});
      if(actor.role!=='member')return send(403,{error:'member_access_required'});
      const response=await quoteFromCentral(central,actor.authSubject||actor.id,body,time);
      return send(response.status,response.body);
    }catch(e){return send(e instanceof core.CommerceError?e.status:503,
      {error:e instanceof core.CommerceError?e.message:'central_quote_unavailable'});}
  }
  if(!preview && req.method==='POST' && path==='/orders'){
    try{
      const session=await withSaveRead(s=>({status:200,body:{actor:sessionActor(s,req,time)}}));
      const actor=session.body?.actor;
      if(!actor)return send(401,{error:'sign_in_required'});
      if(actor.role!=='member')return send(403,{error:'member_access_required'});
      const result=await reserveFromCentral(central,actor.authSubject||actor.id,body,
        req.headers['idempotency-key']);
      return send(result.status,result.body);
    }catch(e){return send(e instanceof core.CommerceError?e.status:503,
      {error:e instanceof core.CommerceError?e.message:'service_unavailable'});}
  }
  if(!preview && req.method==='POST' && path==='/cancel'){
    try{
      const session=await withSaveRead(s=>({status:200,body:{actor:sessionActor(s,req,time)}}));
      const actor=session.body?.actor;
      if(!actor)return send(401,{error:'sign_in_required'});
      if(actor.role!=='member')return send(403,{error:'member_access_required'});
      const result=await cancelFromCentral(central,actor.authSubject||actor.id,body,
        req.headers['idempotency-key']);
      return send(result.status,result.body);
    }catch(e){return send(e instanceof core.CommerceError?e.status:503,
      {error:e instanceof core.CommerceError?e.message:'service_unavailable'});}
