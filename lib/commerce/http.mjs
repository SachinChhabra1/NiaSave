  ...
  try {
    const result = await withSaveState(async s => {
      ...
      try {
        ...
        if(req.method === 'GET' && path === '/catalogue') return out({...core.catalogue(s,SKUS,preview,time),memberAuth:!preview&&passkeysEnabled()?'passkey':'legacy',account:actor && {id:actor.id,name:actor.name,role:actor.role,locationIds:actor.locationIds}});
