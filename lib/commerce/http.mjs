...
export async function commerceHttp(req,res,path,getStaff) {
  ...
  if (process.env.EARN_SOURCE==='central' && ['/earn','/earn/applications'].includes(path)) {
    try {
      ...
      const result=await central.centralMemberRequest(req.method==='GET'?'earn.applications':'earn.apply',actor.id,req.method==='GET'?{}:{jobId:body.jobId,revision:body.revision,consent:body.consent,idempotencyKey:req.headers['idempotency-key']});
      // Preserve previous applications as history; all new writes go only to Central.
      if(req.method==='GET'&&result.status===200)result.body.applications=[...result.body.applications,...auth.body.legacy.filter(a=>!result.body.applications.some(b=>b.id===a.id))];
      return send(result.status,result.body);
