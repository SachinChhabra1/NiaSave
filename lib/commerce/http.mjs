...
import { withLivingState, createContract, cancelBooking } from '../../bison/engine.mjs';
...
const liveReady = () => process.env.COMMERCE_ENABLED === '1' && !DUMMY_DATA && hasDurableStore() && process.env.STAFF_AUTH_REQUIRED === '1' && (process.env.STAFF_TOKEN_SECRET||'').length >= 32 && Boolean(process.env.STAFF_PASSWORD) && (passkeysReady() || (!passkeysEnabled() && /^https:\/\//.test(process.env.COMMERCE_IDENTITY_URL || '') && Boolean(process.env.COMMERCE_IDENTITY_KEY)));
...
export async function commerceHttp(req,res,path,getStaff) {
  ...
  if(!preview && passwordAuthEnabled()){
    ...
    if(otpRequestPath||otpVerifyPath||setPasswordPath){
      ...
      if(otpRequestPath||otpVerifyPath){
        ...
        let lookup={registered:false,subject:null};
        try{lookup=await central.centralMemberPhoneLookup(phone);}
        catch(e){
          if(requireCentralMember()){
            if(e instanceof core.CommerceError)return send(e.status||503,{error:e.message});
            return send(503,{error:'central_member_lookup_unavailable'});
          }
          lookup={registered:false,subject:null};
        }
        if(requireCentralMember()&&!lookup.registered)return send(403,{error:'not_registered'});
        if(otpRequestPath){
          let result;
          try{result=await identity('request',{phone});}
          catch{return send(503,{error:'otp_unavailable'});}
        ...
        try{await identity('verify',{challenge:body.challenge,code:body.code});}
        catch{return send(401,{error:'bad_otp'});}
        let setupToken;
        try{setupToken=issueMemberSetupToken(phone,lookup.subject,time);}
      ...
      const setup=memberSetupFromRequest(req,time);
      ...
      let profile;
      try{profile=buildMemberPasswordProfile(setup.phone,body.password,{subject:setup.subject},time);}
      catch(e){
    ...
    if(!passwordActor){
      if(req.method==='GET' && path==='/catalogue')return send(200,passwordCatalogue(null));
    ...
    }else if(req.method==='GET' && path==='/catalogue' && !liveReady())return send(200,passwordCatalogue(passwordActor));
  ...
  // Keep the existing readiness requirement on actual commerce operations.
  ...
  // Production defaults to Central earn.projection even when EARN_SOURCE is unset.
  // Isolated preview keeps the local ledger unless EARN_SOURCE=central.
  if (central.earnUsesCentral(preview) && ['/earn','/earn/applications'].includes(path)) {
    try {
  ...
  if (path.startsWith('/nests')) {
    try {
      ...
      if(!auth.body.allowed)return send(429,{error:'too_many_attempts'});
      const actor=auth.body.actor;
      // FOUNDER_ASK: Central REQUEST_KINDS has no member-safe vacant-nest / occupancy read.
      // Do not invent vacancy from the local Living book. Preview demo catalogue stays.
      if(!preview&&((req.method==='GET'&&path==='/nests')||(req.method==='POST'&&path==='/nests/availability'))){
        return send(200,nests.unconnectedOccupancy(time,path==='/nests/availability'?body.start:undefined));
      }
      if(!preview&&(path==='/nests/quote'||(req.method==='POST'&&path==='/nests/bookings'))){
        return send(409,{error:'central_occupancy_not_allowlisted',message:'No data yet'});
      }
      const result=await withLivingState(async s=>{
        if(!preview&&s.dummy!==false)return {status:503,body:{error:'preview_data_cannot_go_live'}};
        const out=(body,status=200)=>({status,body});
        if(req.method==='GET'&&path==='/nests')return out(nests.nestCatalogue(s,preview,time));
        if(req.method==='POST'&&path==='/nests/availability')return out(nests.nestCatalogue(s,preview,time,body.start));
        if(req.method==='POST'&&path==='/nests/quote')return out(nests.nestQuote(s,actor,body,preview,time));
        if(req.method==='GET'&&path==='/nests/bookings')return out(nests.ownNests(s,actor,time));
        if(req.method==='POST'&&path==='/nests/bookings')return out(nests.reserveNest(s,actor,body,req.headers['idempotency-key'],preview,time,createContract),201);
  ...
  try {
    const result = await withSaveState(async s => {
      ...
      try {
        ...
        if(req.method === 'GET' && path === '/catalogue') return out({...core.catalogue(s,SKUS,preview,time),memberAuth:!preview&&passwordAuthEnabled()?'password':!preview&&passkeysEnabled()?'passkey':'legacy',...(!preview&&passwordAuthEnabled()?{memberAuthCapabilities:passwordAuthCapabilities()}:{}),account:actor && {id:actor.id,name:actor.name,role:actor.role,locationIds:actor.locationIds}});
