...
export async function commerceHttp(req,res,path,getStaff) {
  ...
  if(!preview && passwordAuthEnabled()){
    if(path.startsWith('/auth/password/')){
      ...
      if(path==='/auth/password/request'||path==='/auth/password/verify'){
        ...
        if(!lookup.registered)return send(403,{error:'member_not_registered'});
        if(path==='/auth/password/request'){
          let result;
          try{result=await identity('request',{phone});}
          catch{return send(503,{error:'otp_provider_unavailable'});}
          if(typeof result.challenge!=='string'||result.challenge.length>300)return send(503,{error:'otp_provider_unavailable'});
          return send(200,{challenge:result.challenge,message:'OTP sent to the registered member phone.'});
        }
        if(typeof body.challenge!=='string'||!/^\d{4,8}$/.test(body.code||''))return send(400,{error:'invalid_code'});
        ...
        return send(200,{ok:true,next:'set_password'},{'set-cookie':memberSetupCookie(setupToken)});
      }
      if(path==='/auth/password/set'){
        const setup=memberSetupFromRequest(req,time);
        if(!setup)return send(401,{error:'setup_expired'});
        if(typeof body.password!=='string'||body.password.length<8||body.password.length>120)return send(400,{error:'invalid_password'});
        ...
        try{profile=buildMemberPasswordProfile(setup.phone,body.password,{subject:setup.subject},time);}
          ...
          if(e?.message==='invalid_phone'||e?.message==='invalid_password')return send(400,{error:e.message});
  ...
  try {
    const result = await withSaveState(async s => {
      ...
      try {
        ...
        if(req.method === 'GET' && path === '/catalogue') return out({...core.catalogue(s,SKUS,preview,time),memberAuth:!preview&&passwordAuthEnabled()?'password':!preview&&passkeysEnabled()?'passkey':'legacy',account:actor && {id:actor.id,name:actor.name,role:actor.role,locationIds:actor.locationIds}});
