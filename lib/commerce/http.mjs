...
export async function commerceHttp(req,res,path,getStaff) {
  ...
  if(!preview && passwordAuthEnabled()){
    ...
    if(!passwordActor){
      if(req.method==='GET' && isTestMemberPath(path)){
        // TEST auto-session / one-shot order / signed read. Real members still 401 below.
      }else if(req.method==='GET' && path==='/catalogue'){
        // Guest browse stays pending. Persist the TEST lane so a signed-in member can order.
        if(durableOrdersReady()){
          try{
            await withSaveState(s=>{
              core.initialise(s,SKUS,preview,time);
    ...
    }else if(req.method==='GET' && path==='/catalogue' && !liveReady() && !durableOrdersReady())return send(200,passwordCatalogue(passwordActor));
  ...
  if (!preview && !liveReady() && !(durableOrdersReady() && (passwordActorNow || path.startsWith('/staff/') || isTestMemberPath(path)))) return send(503,{error:'commerce_not_configured',message:'Ordering is not open yet.'});
    ...
    const result = await withSaveState(async s => {
      ...
      try {
        if(req.method === 'GET' && isTestMemberPath(path)) {
          ...
          const o = s.orders.find(row=>row.id===id && row.source==='commerce' && row.test===true && row.memberId===TEST_MEMBER.id);
        ...
        if(req.method === 'POST' && path === '/orders') {
          ...
          return out(core.reserve(s,actor,body,req.headers['idempotency-key']||(actor?.test===true?TEST_ORDER_IDEMPOTENCY_KEY:undefined),SKUS,preview,time),201);
        }
