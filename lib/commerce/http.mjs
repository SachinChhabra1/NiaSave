...
export async function commerceHttp(req,res,path,getStaff) {
  ...
  if(!preview && passwordAuthEnabled()){
    ...
    if(otpRequestPath||otpVerifyPath||setPasswordPath){
      ...
      let profile;
      try{profile=buildMemberPasswordProfile(setup.phone,body.password,{subject:setup.subject,...(Array.isArray(setup.locationIds)?{locationIds:setup.locationIds,name:setup.name}:{})},time);}
      catch(e){
