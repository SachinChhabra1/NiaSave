...
export async function commerceHttp(req,res,path,getStaff) {
  ...
  if (req.method !== 'GET' && (req.headers.origin !== `https://${req.headers.host}` && !(preview && !hosted && ["http://127.0.0.1:5173", "http://127.0.0.1:8080", `http://${req.headers.host}`, `https://${req.headers.host}`].includes(req.headers.origin)))) return send(403,{error:'same_origin_required'});
