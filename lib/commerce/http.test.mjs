...
test('P5 Help opens a routed case without a reservation and keeps Save order checks',async()=>{
 const state=fresh(),cookie=await login();
 const created=await request('/support',{kind:'move_in',relatedRef:'nest-7',note:'Wrong door'},cookie);
 assert.equal(created.status,201);
 assert.equal(created.body.reference,created.body.id);
 assert.equal(created.body.unit,'nest_operator');
 assert.equal(created.body.relatedRef,'nest-7');
 assert.equal(created.body.refundApproved,false);
 const listed=await request('/support',undefined,cookie);
 assert.equal(listed.status,200);
 assert.equal(listed.body.issues.at(-1).reference,created.body.reference);
 assert.equal(state.commerce.audit.at(-1).action,'support_opened');
 const missing=await request('/support',{kind:'missing_item',orderId:'unknown'},cookie);
 assert.equal(missing.status,400);
 assert.equal(missing.body.error,'order_and_issue_required');
