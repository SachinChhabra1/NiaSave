...
test('HTTP session, role enforcement, no CSRF and persistent duplicate request protection',async()=>{
 const s=fresh();const cookie=await login();confirmPreviewPacks(s);assert.match(cookie,/nia_commerce=/);const cat=await request('/catalogue',undefined,cookie);assert.equal(cat.body.account.role,'member');assert.equal(cat.body.preview,true);assert.equal(cat.headers['cache-control'],'no-store');
