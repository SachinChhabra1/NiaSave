...
test('all literal member UI messages have a translation in each offered non-English language',async()=>{
 ...
 for(const lang of ['hi','bn','ta','kn']){const dict=(await import(`../../commerce-locales/${lang}.js`)).default;const missing=[...keys].filter(key=>!(lang==='hi'&&inlineHindi.has(key))&&(typeof dict[key]!=='string'||!dict[key].trim()));assert.deepEqual(missing,[],lang+' missing translations');}
});
