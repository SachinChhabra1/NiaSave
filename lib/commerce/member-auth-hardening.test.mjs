...
const customerId='customer_'+'c'.repeat(32);
...
globalThis.fetch = async (_url, init = {}) => {
  ...
  if (request.kind === 'identity.verify.confirm') {
    ...
    if(customerMode)return {status:200,json:async()=>({account:{id:customerId,role:'member',kind:'customer',authVersion:'a'.repeat(64),name:'Customer',locationIds:[]},customer:{id:customerId,status:'active',phoneVerified:true}})};
  ...
  if (request.kind === 'member.identity') {
    ...
    if(customerMode)return {status:200,json:async()=>({source:'central',status:'ready',account:{id:customerId,role:'member',kind:'customer',authVersion:(centralMode==='changed-version'?'b':'a').repeat(64),name:'Customer',locationIds:[]},customer:{id:customerId,status:'active',phoneVerified:true}})};
...
test('phone-verified customer keeps its own identity through OTP and password without member-service privileges',()=>otpFixture(async()=>{
  customerMode=true;
  const first=await issueOtpSession();
  assert.equal(first.result.body.account.id,customerId);assert.equal(first.result.body.account.kind,'customer');
  assert.deepEqual(first.result.body.account.locationIds,[]);assert.equal(savedState().commerce.memberPasswords,undefined);
  assert.equal(auth.verifyMemberSession(first.member.slice('nia_member='.length)).kind,'customer');
  const cat=await request('/catalogue',undefined,first.member);assert.equal(cat.status,200);assert.equal(cat.body.account.kind,'customer');
  for(const [path,body] of [['/membership'],['/partners'],['/books'],['/books/plan',{month:'2026-09'}],['/earn'],['/earn/applications',{}],['/nests'],['/nests/bookings',{}],['/recovery',{}]]){
    const before=centralCalls.length,result=await request(path,body,first.member);
    if(result.status===503){assert.equal(result.body.error,'pilot_commitments_paused',path);assert.equal(centralCalls.length,before,'existing global freeze blocks before identity work');}
    else {assert.equal(result.status,403,path);assert.equal(result.body.error,'customer_save_only',path);assert.deepEqual(centralCalls.slice(before).map(c=>c.kind),['member.identity'],'no privileged Central request');}
  }
  assert.equal((await request('/auth/password',{password:'customer-personal-password'},first.cookie)).status,200);
  const profiles=Object.values(savedState().commerce.memberPasswords);assert.equal(profiles.length,1);assert.equal(profiles[0].kind,'customer');assert.equal(profiles[0].subject,customerId);
  const login=await request('/auth/login',{phone:'9876543210',password:'customer-personal-password'});
  assert.equal(login.status,200);assert.equal(login.body.account.id,customerId);assert.equal(login.body.account.kind,'customer');
  centralMode='changed-version';assert.equal((await request('/catalogue',undefined,first.member)).status,401);
  assert.equal((await request('/auth/login',{phone:'9876543210',password:'customer-personal-password'})).status,401);
  centralMode='revoked';assert.equal((await request('/orders',undefined,first.member)).status,401);
  centralMode='unavailable';assert.equal((await request('/orders',undefined,first.member)).status,503);
}));
