 ...
 * Orders persist through withSaveState → nia_runtime_state.
 */
...
export function applyMemberFulfillment(s, time, skus = []) {
  ...
  const c = s.commerce;
  if (!c) return s;
  if (c.config?.verified && c.config.memberFulfillment !== true) return s;
  ...
  c.config = {
    ...
    reserveMinutes: 120,
    products,
    locations: [existing && Date.parse(existing.windowEnd) > time ? { ...existing, modes: existing.modes?.length ? existing.modes : ['pickup'], name: location.name, address: existing.address || location.address } : location],
