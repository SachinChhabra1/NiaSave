...
export function applyMemberFulfillment(s, time, skus = []) {
  ...
  if (staffLocked) {
    if (c.config) {
      c.config.products = overlayCatalogueProducts(c.config.products, s.uploads?.catalogue, { testLane: c.config.testLane === true, skus });
    }
    return s;
  }
  ...
  // Publish only pack-confirmed rows. Pending-pack SKUs (no mill match) stay in
  // fulfillmentProducts for truth checks but are not orderable catalogue products
  // — e.g. bathsoap_pick (nia 70 ≠ mill 48/52/58) and essentials_pick (no mill).
  c.config = {
    verified: true,
    beatDate: s.beat.beatDate,
    reserveMinutes: 120,
    products: ready.map(stampTestProduct),
  ...
  c.config.products = overlayCatalogueProducts(c.config.products, s.uploads?.catalogue, { testLane: true, skus });
  return s;
}
