...
export function applyMemberFulfillment(s, time, skus = []) {
  ...
  c.config = {
    ...
    reserveMinutes: 120,
    // Unconfirmed packs stay on the frozen SKU list and are not orderable.
    // bathsoap_pick has no mill row at nia 70; essentials_pick has no mill category.
  ...
  c.config.products = overlayCatalogueProducts(c.config.products, s.uploads?.catalogue, { testLane: true, skus }).filter(packIsConfirmed);
  return s;
}
