...
test('fulfilment uses only frozen SKU ids and prices; packs come from matching mill rows', () => {
  ...
  assert.equal(packIsConfirmed(products.find(p => p.id === 'bathsoap_pick')), false);
  assert.equal(millPackForSku(SKUS.find(s => s.id === 'bathsoap_pick')), null);
  assert.equal(millPackForSku(SKUS.find(s => s.id === 'essentials_pick')), null);
...
test('applyMemberFulfillment opens Ompal pickup on existing SKUs and does not invent delivery', () => {
  ...
  assert.equal(s.commerce.config.locations[0].sourceSiteCode, 'S01');
  ...
  // Quarantine: pending-pack SKUs must not appear as orderable catalogue products.
  assert.equal(s.commerce.config.products.find(p => p.id === 'essentials_pick'), undefined);
  assert.equal(s.commerce.config.products.find(p => p.id === 'bathsoap_pick'), undefined);
  assert.ok(s.commerce.config.products.every(p => packIsConfirmed(p)));
  assert.ok(s.commerce.config.products.find(p => p.id === 'groundnut_oil'));
  assert.ok(s.commerce.config.products.find(p => p.id === 'nia_bathsoap'));
});
