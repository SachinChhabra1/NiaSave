...
test('applyMemberFulfillment opens Ompal pickup on existing SKUs and does not invent delivery', () => {
  ...
  assert.equal(s.commerce.config.products.some(p => p.id === 'bathsoap_pick'), false);
  assert.equal(s.commerce.config.products.some(p => p.id === 'essentials_pick'), false);
  assert.ok(s.commerce.config.products.every(packIsConfirmed));
  assert.equal(fulfillmentProducts(SKUS).some(p => p.id === 'bathsoap_pick'), true);
  assert.equal(fulfillmentProducts(SKUS).some(p => p.id === 'essentials_pick'), true);
});
