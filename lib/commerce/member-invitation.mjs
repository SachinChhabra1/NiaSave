...
// Member API handlers protect identity; staff/service authorization is separate.
export function memberInvitation(request,env=process.env){
  ...
  if(!['passkey','password','otp'].includes(env.COMMERCE_MEMBER_AUTH))return new Response('Member access is not configured.',{status:503,headers});
