...
test('member invitation protects pages and direct member API aliases',()=>{for(const path of ['/','/commerce.js','/api/commerce/catalogue','/api/api/commerce/catalogue','/api?path=commerce/catalogue','/api/index?path=/commerce/auth/passkey/options','/api/index.mjs?path=api/commerce/catalogue']){assert.equal(memberInvitation(new Request('https://www.nia.test'+path),env).status,401);assert.equal(memberInvitation(new Request('https://www.nia.test'+path,{headers:{authorization:auth}}),env),undefined);}});
