...
// Temporary tester switch. Unset or any value other than "true" skips the
// Central-registered hard stop on identity OTP. Set to "true" to restore it.
...
function accountFromClaims(claims) {
  ...
  return {
    ...
    role: 'member',
    locationIds,
    identitySource: id === 'nia-test-phone-member' ? 'test-phone-otp' : id === 'nia-test-member' ? 'test-auto-session' : 'member-password',
...
export function issueMemberSetupToken(phone, subject, at = Date.now(), env = process.env, account = null) {
  ...
  const payload = Buffer.from(JSON.stringify({
    ...
    ...(account ? {name: typeof account.name==='string'?account.name.slice(0,120):'Nia member', locationIds:Array.isArray(account.locationIds)?account.locationIds.filter(validSubject):[]} : {}),
    iat: Math.floor(at / 1000),
...
export function verifyMemberSetupToken(raw, at = Date.now(), env = process.env) {
  ...
  try {
    ...
    if (!claims.exp || claims.exp <= Math.floor(at / 1000)) return null;
    return {phone, subject: validSubject(claims.subject) ? claims.subject : null, ...(Array.isArray(claims.locationIds)?{locationIds:claims.locationIds.filter(validSubject),name:typeof claims.name==='string'?claims.name.slice(0,120):'Nia member'}:{})};
  } catch {
    return null;
  }
}
...
export function buildMemberPasswordProfile(phone, password, {subject = null, locationIds = ['S01'], name = 'Nia member'} = {}, at = Date.now(), env = process.env) {
  const normalized = normalizeMemberPhone(phone);
