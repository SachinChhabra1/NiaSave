...
import {identityClientIp} from './central-client.mjs';
...
export const PERSONAL_PASSWORD_KDF = Object.freeze({
  scheme: 'scrypt', version: 1, N: 32768, r: 8, p: 1,
  keyLength: 32, saltBytes: 16, maxmem: 64 * 1024 * 1024
});
...
export function passwordAuthCapabilities(env = process.env) {
  return {
    mode: otpPrimaryAuth(env) ? 'otp' : 'password_otp',
    ...(otpPrimaryAuth(env) ? {primaryMethod:'whatsapp_otp',passwordOptional:true,passwordSetupRequiresOtp:true,passwordSetupWindowSeconds:MEMBER_SETUP_TTL_SECONDS} : {}),
    entryPath: '/',
    hashTabs: ['live', 'earn', 'shop', 'send'],
    apiBasePath: '/api/commerce',
    registeredPhoneOnly: !otpPrimaryAuth(env) && requireCentralMember(env),
    otpRequestPath: '/api/commerce/auth/request',
    otpVerifyPath: '/api/commerce/auth/verify',
    setPasswordPath: otpPrimaryAuth(env) ? '/api/commerce/auth/password' : '/api/commerce/auth/set-password',
...
export function otpPrimaryAuth(env = process.env) {
  return String(env.COMMERCE_MEMBER_AUTH || '') === 'otp';
}

export function passwordAuthEnabled(env = process.env) {
  return String(env.SESSION_SECRET || '').length >= 16
    && (String(env.MEMBER_PASSWORD || '').length >= 8 || ['password','otp'].includes(String(env.COMMERCE_MEMBER_AUTH || '')));
}
...
function validPersonalPasswordCredential(credential) {
  return credential && typeof credential === 'object' && !Array.isArray(credential)
    && credential.scheme === PERSONAL_PASSWORD_KDF.scheme
    && credential.version === PERSONAL_PASSWORD_KDF.version
    && credential.N === PERSONAL_PASSWORD_KDF.N
    && credential.r === PERSONAL_PASSWORD_KDF.r
    && credential.p === PERSONAL_PASSWORD_KDF.p
    && credential.keyLength === PERSONAL_PASSWORD_KDF.keyLength
    && credential.maxmem === PERSONAL_PASSWORD_KDF.maxmem
    && typeof credential.salt === 'string'
    && Buffer.from(credential.salt, 'base64url').length === PERSONAL_PASSWORD_KDF.saltBytes
    && typeof credential.verifier === 'string'
    && Buffer.from(credential.verifier, 'base64url').length === PERSONAL_PASSWORD_KDF.keyLength;
}

function personalPasswordCredential(password) {
  const salt = randomBytes(PERSONAL_PASSWORD_KDF.saltBytes);
  const verifier = scryptSync(password, salt, PERSONAL_PASSWORD_KDF.keyLength, {
    N: PERSONAL_PASSWORD_KDF.N, r: PERSONAL_PASSWORD_KDF.r,
    p: PERSONAL_PASSWORD_KDF.p, maxmem: PERSONAL_PASSWORD_KDF.maxmem
  });
  return {
    scheme: PERSONAL_PASSWORD_KDF.scheme, version: PERSONAL_PASSWORD_KDF.version,
    salt: salt.toString('base64url'), N: PERSONAL_PASSWORD_KDF.N,
    r: PERSONAL_PASSWORD_KDF.r, p: PERSONAL_PASSWORD_KDF.p,
    keyLength: PERSONAL_PASSWORD_KDF.keyLength, maxmem: PERSONAL_PASSWORD_KDF.maxmem,
    verifier: verifier.toString('base64url')
  };
}

export function isLegacyPersonalPasswordProfile(profile) {
  return Boolean(profile?.passwordDigest && !profile?.passwordCredential);
}

export function profilePasswordMatches(phone, submitted, profile, env = process.env) {
  const credential = profile?.passwordCredential;
  const actual = typeof submitted === 'string' ? submitted : '';
  if (credential) {
    if (!validPersonalPasswordCredential(credential) || actual.length < 8 || actual.length > 120) return false;
    try {
      const derived = scryptSync(actual, Buffer.from(credential.salt, 'base64url'), credential.keyLength, {
        N: credential.N, r: credential.r, p: credential.p, maxmem: credential.maxmem
      });
  ...
  const legacyActual = passwordProfileDigest(phone, submitted, env);
  if (!expected || !legacyActual) return false;
  const left = Buffer.from(legacyActual);
...
function memberAccountFromProfile(phone, profile, env = process.env) {
  ...
  return {
    ...
    ...(profile?.identitySource==='central-whatsapp'?{kind:profile.kind,authVersion:profile.authVersion,locationModes:profile.locationModes||{},locationPinCodes:profile.locationPinCodes||{}}:{}),
...
function accountFromClaims(claims) {
  ...
  return {
    ...
    role: 'member',
    locationIds,
    ...(claims.identitySource==='central-whatsapp'?{kind:claims.kind,authVersion:claims.authVersion,locationModes:claims.locationModes||{},locationPinCodes:claims.locationPinCodes||{}}:{}),
    ...(claims.identitySource==='central-whatsapp'&&/^[A-Za-z0-9_-]{43}$/.test(claims.passwordSetupGrantId||'')?{passwordSetupGrantId:claims.passwordSetupGrantId}:{}),
...
export function issueMemberSession(accountOrAt = MEMBER_ACCOUNT, maybeAt = Date.now(), env = process.env, ttlSeconds = MEMBER_SESSION_TTL_SECONDS) {
  ...
  const payload = Buffer.from(JSON.stringify({
    ...
    ...(account.identitySource==='central-whatsapp'?{identitySource:'central-whatsapp',kind:account.kind,authVersion:account.authVersion,locationModes:account.locationModes||{},locationPinCodes:account.locationPinCodes||{}}:{}),
    ...(account.identitySource==='central-whatsapp'&&/^[A-Za-z0-9_-]{43}$/.test(account.passwordSetupGrantId||'')?{passwordSetupGrantId:account.passwordSetupGrantId}:{}),
...
// The one-time setup grant is keyed by an HMAC of the signed cookie. The raw
// cookie remains client-held and is never copied into runtime state.
export function memberSetupGrantId(raw, env = process.env) {
  const secret = String(env.SESSION_SECRET || '');
  if (!raw || secret.length < 16) return '';
  return digestText('member-setup-grant-v1\n' + raw, secret);
}
...
export function verifyMemberSetupToken(raw, at = Date.now(), env = process.env) {
  ...
  try {
    ...
    if (!claims.exp || claims.exp <= Math.floor(at / 1000)) return null;
    return {phone, subject: validSubject(claims.subject) ? claims.subject : null, ...(Array.isArray(claims.locationIds)?{identitySource:claims.identitySource,kind:claims.kind,authVersion:claims.authVersion,locationModes:claims.locationModes||{},locationPinCodes:claims.locationPinCodes||{},locationIds:claims.locationIds.filter(validSubject),name:typeof claims.name==='string'?claims.name.slice(0,120):'Nia member'}:{})};
  } catch {
    return null;
  }
}
...
export function memberSetupGrantIdFromRequest(req, env = process.env) {
  return memberSetupGrantId(cookieValue(req, MEMBER_SETUP_COOKIE), env);
}
...
export function publicMemberAccount(actor) {
  return actor && {id: actor.id, name: actor.name, role: actor.role, ...(actor.kind==='customer'?{kind:'customer'}:{}), locationIds: actor.locationIds, ...(actor.test===true?{test:true}:{})};
}

// The durable HTTP gate is authoritative for real personal-password logins.
// Keep this process-local limit as defense-in-depth for the legacy shared
// MEMBER_PASSWORD fallback and callers that do not pass through that gate.
...
export function passwordCatalogue(account, env = process.env, stockRows = []) {
  ...
  return {
    preview: false,
    memberAuth: otpPrimaryAuth(env) ? 'otp' : 'password',
...
function guestTestLane(stockRows = []) {
  const products = decorateBrowseProducts(
    fulfillmentProducts(SKUS).map(product => packIsConfirmed(product) ? stampTestProduct(product) : product),
    stockRows
  );
  return {
    products,
...
export function buildMemberPasswordProfile(phone, password, {subject = null, locationIds = ['S01'], name = 'Nia member',identitySource=null,kind=null,authVersion=null,locationModes={},locationPinCodes={}} = {}, at = Date.now(), env = process.env) {
  const normalized = normalizeMemberPhone(phone);
  if (!normalized) throw new Error('invalid_phone');
  if (typeof password !== 'string' || password.length < 8 || password.length > 120) throw new Error('invalid_password');
  if (String(env.SESSION_SECRET || '').length < 16) throw new Error('member_password_not_configured');
  return {
    passwordCredential: personalPasswordCredential(password),
    ...(identitySource==='central-whatsapp'?{identitySource,kind:kind==='customer'?'customer':undefined,authVersion,locationModes,locationPinCodes}:{}),
...
export function memberPasswordLogin(body, req, at = Date.now(), env = process.env, profile = null) {
  ...
  // Match the durable gate's trusted client identity. On Vercel, a missing or
  // invalid overwritten edge header deliberately falls into one shared bucket;
  // outside Vercel/test callers retain their socket identity.
  if (!allowMemberLoginAttempt(identityClientIp(req, env), at)) return {status: 429, body: {error: 'too_many_attempts'}};
  ...
  let account = null;
  if (phone && (profile?.passwordCredential || profile?.passwordDigest)) {
