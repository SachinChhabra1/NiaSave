...
function accountFromClaims(claims) {
  ...
  return {
    ...
    role: 'member',
    locationIds,
    identitySource: claims.test === true && id === 'nia-test-member' ? 'test-auto-session' : 'member-password',
    ...(authSubject ? {authSubject} : {}),
    ...(verifiedPhone ? {verifiedPhone} : {}),
    ...(claims.test === true && id === 'nia-test-member' ? {test: true} : {})
  };
}
...
export function issueMemberSession(accountOrAt = MEMBER_ACCOUNT, maybeAt = Date.now(), env = process.env, ttlSeconds = MEMBER_SESSION_TTL_SECONDS) {
  ...
  const payload = Buffer.from(JSON.stringify({
    ...
    role: 'member',
    ...(account.test === true && account.id === 'nia-test-member' ? {test: true} : {}),
...
export function passwordCatalogue(account, env = process.env, stockRows = []) {
  ...
  return {
    ...
    products: browse ? decorateBrowseProducts(publicBrowseProducts(SKUS), stockRows) : [],
    locations: [],
