...
test('phone OTP bootstrap rejects unregistered numbers before OTP send and allows registered numbers to set a remembered password', async () => {
  ...
  try {
    ...
    const weak = await request('/auth/password', {password: 'short'}, {cookie: setupCookie});
    assert.equal(weak.status, 400);
    assert.equal(weak.body.error, 'weak_password');

    const configured = await request('/auth/password', {phone: '9876543210', password: 'new-member-password', remember: true}, {cookie: setupCookie});
