...
// M0 never bootstraps an operational book during an HTTP read. Authentication
// regressions run against an existing empty book, as in the deployed service.
const {resetDummy}=await import('../../rabbit/engine.mjs');
const {ordersById,ordersByCode,orderIdsByStop,...existingBook}=structuredClone(resetDummy());
const rows = new Map([[process.env.NIA_RUNTIME_STATE_KEY,{state_value:existingBook,version:1}]]);
...
async function request(path, body, {cookie = '', origin = 'https://www.nia.test', headers = {}} = {}) {
  ...
  // The read adapter is exercised separately with signed Central snapshots.
  // These identity cases need a Central verified empty catalogue, never a local book.
  globalThis.fetch=async (url,init={})=>{
    const kind=typeof init.body==='string' ? JSON.parse(init.body)?.request?.kind : null;
    if(url==='https://central.invalid/api/service/member' && kind==='save.locations')
      return {status:200,json:async()=>({locations:[],asOf:new Date().toISOString(),
        freshnessSeconds:60,pilotOpen:false})};
    if(url==='https://central.invalid/api/service/member' && kind==='save.orders')
      return {status:200,json:async()=>({orders:[],nextCursor:null,asOf:new Date().toISOString()})};
    return previousFetch(url,init);
  };
  try{await commerceHttp(req, res, path, async () => null);}
...
test('wrong password is rejected; correct password sets a signed stay-signed-in cookie; cookie authenticates the next request', async () => {
  ...
  assert.ok(anonymous.body.products.every(p => p.test !== true));
  assert.ok(anonymous.body.locations.every(l => l.test !== true));
  assert.equal((await request('/orders')).status, 401);
...
test('phone login still identifies the member while M0 refuses commitments and TEST catalogue rows', async () => {
  resetMemberLoginAttempts();
  ...
  const catalogue=await request('/catalogue',undefined,{cookie});
  assert.equal(catalogue.status,200,catalogue.body?.error);
  assert.equal(catalogue.body.onlinePayment,false);
  assert.ok(catalogue.body.products.every(p=>p.test!==true));
  assert.ok(catalogue.body.locations.every(p=>p.test!==true));
  for(const path of ['/quote','/orders']){
    const result=await request(path,{locationId:'S01',lines:[{id:'groundnut_oil',qty:1}]},{cookie});
    assert.equal(result.status,503);assert.equal(result.body.error,'pilot_commitments_paused');
  }
});
