...
test('wrong password is rejected; correct password sets a signed stay-signed-in cookie; cookie authenticates the next request', async () => {
  ...
  assert.equal(anonymous.body.products.find(p => p.id === 'groundnut_oil').pack, 'Pack size to be confirmed');
  assert.deepEqual(anonymous.body.locations, []);
  assert.equal((await request('/orders')).status, 401);
...
test('phone OTP bootstrap rejects unregistered numbers before OTP send when the Central-member gate is on', async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (url === 'https://central.invalid/api/service/member') {
      ...
      if (kind === 'identity.verify.confirm') {
        ...
        return code === '123456'
          ? {status: 200, json: async () => ({token: 'central-session', account: {id: 'member-subject-1'}})}
  ...
  try {
    ...
    assert.equal(calls.some(entry => entry.body?.request?.kind === 'identity.verify.request' && entry.body?.request?.phone === '+919999999999'), false);
...
test('phone after Continue keys the member; mill pack order persists with member id and no online pay', async () => {
  ...
  assert.equal(guestAfter.body.products.find(p => p.id === 'groundnut_oil').pack, 'Pack size to be confirmed');
  assert.deepEqual(guestAfter.body.locations, []);
