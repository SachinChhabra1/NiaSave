...
test('phone OTP bootstrap rejects unregistered numbers before OTP send and allows registered numbers to set a remembered password', async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (url === 'https://identity.invalid/verify') return Response.json({ok: true});
    throw new Error('unexpected_fetch_call');
  };
  try {
    const denied = await request('/auth/password/request', {phone: '9999999999'});
    assert.equal(denied.status, 403);
    assert.equal(denied.body.error, 'member_not_registered');
    assert.equal(calls.some(entry => entry.url === 'https://identity.invalid/request' && entry.body?.phone === '+919999999999'), false);

    const requested = await request('/auth/password/request', {phone: '9876543210'});
    assert.equal(requested.status, 200);
    assert.equal(typeof requested.body.challenge, 'string');

    const verified = await request('/auth/password/verify', {phone: '9876543210', challenge: requested.body.challenge, code: '123456'});
    ...
    const configured = await request('/auth/password/set', {password: 'new-member-password', remember: true}, {cookie: setupCookie});
