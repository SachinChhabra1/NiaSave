...
test('phone submission uses OTP on mixed password+OTP catalogue and explicit password still signs in',async()=>{
 ...
 const run=new Function('form','fields','cat','api','finishMemberSession','e164In','errorText','usesPasswordAuth','usesPhoneOtpFlow','showPasswordLogin','submitAuthPaths','otpRequestPaths','show','t','verifyFormMarkup','entryLanguage','esc','loginPath','passwordBody',`return (async()=>{let authPhone='',challenge='';${src.slice(start,end)}})()`);
 const catalogue={memberAuth:'password',memberAuthCapabilities:{mode:'password_otp',otpRequestPath:'/auth/request'}};
 const calls=[];
 const args=[catalogue,async(path)=>{calls.push(path);return{};},async()=>{calls.push('session');},e164In,x=>x,usesPasswordAuth,usesPhoneOtpFlow,()=>{throw Error('wrong_password_dialog');},submitAuthPaths,otpRequestPaths,()=>calls.push('code-screen'),t,verifyFormMarkup,()=>'',x=>x,loginPath,passwordBody];
...
test('fresh password submission validates locally and sends normalized phone with password', async () => {
  ...
  const calls = [];
  const args = [catalogue, async (path, body) => { calls.push({path, body}); return {}; }, async () => { calls.push({path: 'session'}); }, e164In, x => x, usesPasswordAuth, usesPhoneOtpFlow, () => {}, submitAuthPaths, otpRequestPaths, () => {}, t, verifyFormMarkup, () => '', x => x, loginPath, passwordBody];
