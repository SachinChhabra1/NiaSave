...
test('language lives in chrome; first land is the Apple home units', () => {
  ...
  assert.match(src, /pickingLang=false/);
  ...
  assert.match(src, /class="mesha-home apple-home"/);
});
...
test('temptation home is one Live desire plus a secondary Earn/Save/Send rail', () => {
  ...
  assert.match(src, /class="mesha-rail apple-pair"/);
