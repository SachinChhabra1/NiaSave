...
test('bag copy matches Mesha locks: one title, empty hard, Continue CTA', () => {
  ...
  assert.match(src, /shopSourceCopy\(cat\?\.owner\)\.review/);
  assert.equal(shopSourceCopy().review,'Final price and availability are checked by Central at review.');
  assert.equal(shopSourceCopy('niasave').review,'Final price and availability are checked by NiaSave at review.');
  assert.doesNotMatch(src.slice(src.indexOf('function bagChip('),src.indexOf('function bagLine(')),/money\(total\(\)\)/);
  assert.match(src, /Phone only on the next step\. Nothing to pay online\./);
  assert.match(src, /Enter your phone\. Pay when you collect\./);
  assert.doesNotMatch(src, /We'll send a code to reserve this bag/);
...
test('Save aisle keeps short pills and category tiles, not a product rail', () => {
