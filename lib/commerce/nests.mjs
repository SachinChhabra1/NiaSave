...
function offers(s, preview, time) {
  if (preview && !s.memberCatalogue) return s.studios.filter(st => st.capacity > 0).filter((st, i, all) => all.findIndex(v => v.siteId === st.siteId) === i).slice(0, 3).map(st => ({
  ...
  if ((!preview && s.dummy !== false) || s.memberCatalogue?.verified !== true) return [];
  return (s.memberCatalogue.offers || []).filter(o => Date.parse(o.validUntil) > time && s.studios.some(st => st.id === o.studioId));
}

export function configureNests(s, body, staff, time, preview = false) {
  if (staff?.role !== 'admin') fail('admin_access_required', 403);
  if (!preview && s.dummy !== false) fail('preview_data_cannot_go_live', 409);
  ...
  const clean = body.offers.map(o => {
    if(!preview&&o.centralSource?.preview===true)fail('preview_data_cannot_go_live',409);
    ...
    return { ...(o.centralSource?.system==='rafiqi-central'?{sourceSiteCode:String(o.sourceSiteCode||'').slice(0,100),centralSource:{system:'rafiqi-central',siteCode:String(o.sourceSiteCode||'').slice(0,100),pulledAt:String(o.centralSource.pulledAt||'').slice(0,40),preview:o.centralSource.preview===true}}:{}),studioId:studio.id, nestIds:o.nestIds, name:o.name, address:o.address, rent:round(o.rent), deposit:round(o.deposit), taxPct:o.taxPct, holdHours:o.holdHours, terms:o.terms, details:o.details, validUntil:new Date(o.validUntil).toISOString() };
  });
  const staffStudios=Object.fromEntries(Object.entries(body.staffStudios||{}).map(([id,studios])=>{if(!/^[a-zA-Z0-9_-]{1,100}$/.test(id)||!Array.isArray(studios)||!studios.every(v=>s.studios.some(st=>st.id===v)))fail('invalid_staff_studios');return [id,[...new Set(studios)]];}));
  s.memberCatalogue = { staffStudios,verified:true, offers:clean, verifiedBy:staff.id, verifiedAt:new Date(time).toISOString() };
...
// Member Live vacant inventory is not an allowlisted Central kind. Honest empty only.
export function unconnectedOccupancy(time, start) {
  const min = day(time);
  let chosen = min;
  try { if (start !== undefined) chosen = dates(start, time).start; } catch { chosen = min; }
  return {
    preview: false,
    start: chosen,
    end: plus(chosen, 30),
    ...
    offers: [],
    source: 'unconnected',
    reason: 'central_occupancy_not_allowlisted',
    error: 'No data yet'
  };
}
...
export function ownNests(s, actor, time) {
  if(actor?.role !== 'member') fail(actor?'member_access_required':'sign_in_required',actor?403:401);
  expireNests(s,time);
  return {bookings:s.bookings.filter(b=>b.source==='member_storefront'&&ownsMemberRecord(actor,b.memberId)).map(b=>publicBooking(s,b))};
}
export function reserveNest(s, actor, body, key, preview, time, createContract) {
  ...
  const matches=s.bookings.filter(b=>b.source==='member_storefront'&&ownsMemberRecord(actor,b.memberId)&&b.memberReservation?.key===key);
  if(matches.length>1)fail('history_reconciliation_required',409);const existing=matches[0];
  ...
  if(s.bookings.some(b=>ownsMemberRecord(actor,b.memberId)&&overlaps(b,q.start,q.end)))fail('existing_nest_stay',409);
...
export function cancelNest(s, actor, body, time, cancelBooking) {
  if(actor?.role!=='member')fail(actor?'member_access_required':'sign_in_required',actor?403:401);
  expireNests(s,time);
  const b=s.bookings.find(b=>b.id===body.bookingId&&ownsMemberRecord(actor,b.memberId)&&b.source==='member_storefront');
