...
test('M0 does not release unpaid holds locally; paid cancellation remains guarded',()=>{
 ...
 s.collectionPayments=[];nests.expireNests(s,time);assert.equal(b.status,'reserved');assert.equal(s.contracts[0].status,'pending');
});
...
test('concurrent frozen stay reads preserve the existing reservation and its payment state',async()=>{
 const stateValue=state();
 const booking={id:'booking',source:'member_storefront',memberId:'one',studioId:'s1',
   nestId:'N1',contractId:'contract',status:'reserved',arrive:'2026-09-08',depart:'2026-10-08',
   memberReservation:{expiresAt:'2026-09-08T00:00:00Z',
     quote:{name:'Studio',address:'Address',total:2500}}};
 stateValue.bookings=[booking];stateValue.contracts=[{id:'contract',status:'pending'}];
 stateValue.collectionPayments=[{contractId:'contract',amount:500,reference:'receipt'}];
 const before=structuredClone(stateValue);
 const views=await Promise.all(Array.from({length:8},async()=>nests.ownNests(stateValue,actor,time)));
 assert.equal(views.length,8);
 assert.ok(views.every(v=>v.bookings[0].paidAmount===500&&v.bookings[0].canCancel===false));
 assert.deepEqual(stateValue,before);
});
