...
test('unconnected occupancy is honest empty and never invents vacant counts',()=>{
 const empty=nests.unconnectedOccupancy(time);
 assert.deepEqual(empty.offers,[]);
 assert.equal(empty.reason,'central_occupancy_not_allowlisted');
 assert.equal(empty.error,'No data yet');
 assert.equal(empty.preview,false);
 assert.equal(empty.source,'unconnected');
 assert.equal('available' in empty,false);
 const dated=nests.unconnectedOccupancy(time,'2026-09-10');
 assert.equal(dated.start,'2026-09-10');
 assert.equal(dated.offers.length,0);
});
