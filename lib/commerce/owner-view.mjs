import {withSaveRead as withSaveState, SKUS, DUMMY_DATA} from '../../rabbit/engine.mjs';
import {withLivingRead as withLivingState} from '../../bison/engine.mjs';
...
export async function ownerViewHttp(req,res,path,getStaff,dependencies={}) {
  ...
  try {
    if(process.env.NIASAVE_OWNER_VIEW!=='1')return send(404,{error:'not_found'});
    ...
    if(req.method!=='GET'||(Number.isFinite(contentLength)&&contentLength>0))return send(405,{error:'owner_view_read_only'});
