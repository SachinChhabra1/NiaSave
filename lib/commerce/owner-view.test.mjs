...
process.env.NIASAVE_OWNER_VIEW='1';
...
test('router verifies actual signed staff tokens including rewritten route; open desk cannot enter',async()=>{
  for(const url of ['/v1/staff/storefront/catalogue','/api?path=v1/staff/storefront/catalogue']) {
    ...
    assert.equal((await registerStaffSession(token)).ok,true);
    const res=response();await handler({url,method:'POST',headers:{authorization:'Bearer '+token}},res);assert.equal(res.status,405);
  }
});
...
test('owner view refuses a GET that carries a body',async()=>{
  const res=response();
  await ownerViewHttp({method:'GET',url:'/v1/staff/storefront/catalogue',headers:{'content-length':'12'}},res,'/catalogue',()=>admin);
  assert.equal(res.status,405);
});
test('owner storefront is off unless the capability flag is set',async()=>{
  const prev=process.env.NIASAVE_OWNER_VIEW;
  process.env.NIASAVE_OWNER_VIEW='0';
  try {
    const res=response();
    await ownerViewHttp({method:'GET',url:'/v1/staff/storefront/catalogue',headers:{}},res,'/catalogue',()=>admin);
    assert.equal(res.status,404);
  } finally {
    process.env.NIASAVE_OWNER_VIEW=prev;
  }
});
