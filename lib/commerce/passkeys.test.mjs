...
test('passkey adapter binds ceremony to an HTTP-only cookie and never returns Central session secrets in JSON',async()=>{
 ...
 try{
  const options=await preparePasskeys(req(),'/auth/passkey/options',{mode:'register',setupToken:token});assert.equal(options.status,200);assert.equal(options.body.challengeId,undefined);assert.match(options.headers['set-cookie'],/HttpOnly; Secure; SameSite=Strict/);assert.match(options.headers['set-cookie'],/Max-Age=300$/);
  const verified=await preparePasskeys(req('nia_ceremony='+challenge+'.'+binding),'/auth/passkey/verify',{challengeId:'forged',binding:'forged',response:{id:'credential'}});assert.equal(verified.status,200);assert.equal(verified.body.token,undefined);assert.equal(verified.body.account.authSubject,undefined);assert.equal(calls[1].binding,binding);assert.equal(calls[1].challengeId,challenge);assert.match(verified.headers['set-cookie'][0],/pk-/);assert.match(verified.headers['set-cookie'][0],/HttpOnly; Secure; SameSite=Strict; Max-Age=43200$/);assert.match(verified.headers['set-cookie'][1],/Max-Age=0$/);
