...
// Public bag identity is stable per physical site/SKU, even when suppliers reuse product IDs.
export const saveProductId=item=>'save_'+hash([item.siteCode,item.sku]);
...
export function catalogueFromBook(book,context,now=Date.now()){
  ...
  const products=book.inventory.filter(i=>i.active&&locations.some(l=>l.sourceSiteCode===i.siteCode)&&
    fresh(i.countedAt,86400000,now)&&fresh(i.priceVerifiedAt,30*86400000,now)).map(i=>({
      id:saveProductId(i),sourceProductId:i.productId,sourceSku:i.sku,sourceSiteCode:i.siteCode,name:i.name,pack:i.pack,pricePaise:i.pricePaise,
...
function quoteValue(book,context,body,now){
  ...
  const lines=body.lines.map(line=>{
    ...
    // Existing bags/quotes keep working only at their explicitly selected site.
    const item=book.inventory.find(i=>i.siteCode===loc.siteCode&&saveProductId(i)===line.id)||
      (!/^save_[a-f0-9]{64}$/.test(line.id)&&book.inventory.find(i=>i.siteCode===loc.siteCode&&i.productId===line.id));
    ...
    const physicalKey=JSON.stringify([item.siteCode,item.sku]);
    if(physicalItems.has(physicalKey))fail('duplicate_bag_item');
    physicalItems.add(physicalKey);
    if(item.onHand-held(book,item,now)<line.qty)fail('insufficient_stock',409);
    return {id:line.id,sku:item.sku,name:item.name,pack:item.pack,qty:line.qty,
...
export function staffSaveSnapshot(book,actor,now=Date.now()){
  ...
  return {...base,inventory:inventory.map(i=>({...clone(i),reserved:held(book,i,now),available:Math.max(0,i.onHand-held(book,i,now))})),
    orders:orders.map(o=>{const {pickupCode,...view}=publicSaveOrder(o,now);return {...view,
      ...(actor.role==='admin'&&actor.customerSubjects===true&&/^customer_[a-f0-9]{32}$/.test(o.memberSubject)?{customerSubject:o.memberSubject}:{}),
