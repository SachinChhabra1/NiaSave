...
test('mounted Shop notice follows Nia stock and sign-in without opening controls',()=>{
  ...
  context.account=null;context.page='account';context.cat.capabilities={saveShoppingOpen:true,saveReservations:false};
  vm.runInNewContext(code+';applyCommitmentGate();',context);
  assert.match(note.textContent,/^Sign in/,'guest on account/login knows shopping needs sign-in, not that all services are paused');
  assert.equal(button.disabled,true,'public feature flag cannot enable authenticated reservation controls');
  context.page='live';vm.runInNewContext(code+';applyCommitmentGate();',context);assert.equal(note.textContent,'other paused');
  context.cat.capabilities.saveShoppingOpen=false;context.page='account';vm.runInNewContext(code+';applyCommitmentGate();',context);assert.equal(note.textContent,'all paused');
});
...
const {SAVE_BOOK_KEY,SAVE_SMOKE_BOOK_KEY}=await import('./save-repository.mjs');
...
store.useSqlClientForTests(async(strings,...values)=>{
  ...
  if(query.includes('UPDATE nia_runtime_state')){
    ...
    if(![SAVE_BOOK_KEY,SAVE_SMOKE_BOOK_KEY,'niasave-save-auth-v1'].includes(values[1]))legacyWrites++;
...
test('actual signed gateway keeps smoke inventory in fixed private storage and denies completion and unauthenticated selection',async()=>{
  process.env.NIASAVE_SAVE_ENABLED='1';
  const guest=await request('/catalogue',{method:'GET',mounted:true});assert.equal(guest.body.capabilities.saveShoppingOpen,true);assert.equal(guest.body.capabilities.saveReservations,false);assert.deepEqual(guest.body.products,[]);
  const ordinary=structuredClone(rows.get(SAVE_BOOK_KEY));
  const customer='customer_'+'a'.repeat(32),site='production-smoke-pickup';
  const grant={id:'production-smoke-v1',subject:customer,expiresAt:new Date(Date.now()+3600000).toISOString()};
  const body={_siteScope:[{siteCode:site,name:'TEST pickup',theatre:'TEST'}],_testContext:grant};
  let nonce=0;
  async function signed(action,payload=body,role='admin',badSignature=false){
    const envelope={actor:{id:'central-smoke-admin',role},line:'save',action,body:payload,at:Date.now(),nonce:'fixture-smoke-gateway-'+(++nonce)};
    const raw=JSON.stringify(envelope),req=Readable.from([Buffer.from(raw)]);
    Object.assign(req,{url:'/api/central/commerce',method:'POST',headers:{'x-central-signature':badSignature?'bad':centralSignature(raw,process.env.CENTRAL_COMMERCE_KEY)},socket:{remoteAddress:'127.0.0.1'}});
    let result;await handler(req,{writeHead(status){result={status};},end(raw){result.body=JSON.parse(raw);}});return result;
  }
  assert.equal((await signed('snapshot',body,'admin',true)).status,401);
  assert.equal((await signed('snapshot',body,'operator')).status,403);
  assert.equal((await signed('snapshot',{...body,_testContext:{...grant,id:'choose-a-book'}})).status,403);
  assert.equal((await signed('snapshot',{...body,_testContext:{...grant,expiresAt:new Date(Date.now()-1).toISOString()}})).status,403);
  const smokeRow={...inventoryRow,siteCode:site,name:'TEST rice',sku:'test-rice',productId:'test-rice'};
  const preview=await signed('inventory-preview',{...body,rows:[smokeRow]});assert.equal(preview.status,200);
  const publish=await signed('inventory-publish',{...body,rows:[smokeRow],previewHash:preview.body.previewHash,idempotencyKey:'fixture-smoke-publish-01'});
  assert.equal(publish.status,200);assert.ok(rows.has(SAVE_SMOKE_BOOK_KEY));
  const snapshot=await signed('snapshot');assert.equal(snapshot.status,200);assert.equal(snapshot.body.test,true);assert.equal(snapshot.body.inventory[0].name,'TEST rice');assert.equal(snapshot.body.capabilities.complete,false);
  assert.equal((await signed('complete',{...body,orderId:'any'})).body.error,'test_payment_disabled');
  assert.deepEqual(rows.get(SAVE_BOOK_KEY),ordinary,'normal inventory/order book untouched');
  const regular=await signed('snapshot',{_siteScope:scope.sites});assert.equal(regular.body.test,undefined);assert.equal(regular.body.inventory.some(row=>row.siteCode===site),false);
  assert.equal(legacyWrites,0);
});
