...
test('live Shop renders only the Central catalogue, without the local mill rows', () => {
  const src=fs.readFileSync(new URL('../../commerce.js',import.meta.url),'utf8');
  const shop=src.slice(src.indexOf('function shop(){'),src.indexOf('function showBuy('));
  const view=fs.readFileSync(new URL('../../commerce-shop-v2.js',import.meta.url),'utf8');
  assert.match(shop,/shopCatalogueMarkup/);
  assert.doesNotMatch(shop,/manufacturersFor|saveMakers|saveAisle/);
  assert.match(view,/data-action="open-aisle"/);
  assert.match(view,/id="save-search-form"/);
  assert.match(view,/Unit price not supplied by Central/);
});
