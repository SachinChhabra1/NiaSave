...
export function packIsConfirmed(product = {}) {
  const pack = typeof product.pack === 'string' ? product.pack.trim() : '';
  return Boolean(pack) && pack !== PENDING_PACK;
}

export function canReserveProduct(product = {}) {
  const price = Number(product.price ?? product.nia);
  const available = Number(product.available);
  if (!product?.id || !packIsConfirmed(product)) return false;
