...
test('photo tap opens a buy sheet with Add, not a photo-only dialog', () => {
  ...
  assert.match(commerce, /if\(action==='open-buy'\|\|action==='detail'\)\{return showBuy\(id\);\}/);
  assert.match(commerce, /data-buy-id/);
  assert.match(commerce, /data-action="add"/);
  assert.match(commerce, /shopPrice\(p,\{t,esc,money,sourceOwner:cat\?\.owner\}\)/);
  assert.match(commerce, /shopPhoto\(p,\{esc,t,name:title\(p\),sourceOwner:cat\?\.owner\}\)/);
  assert.match(commerce, /Pay when you collect/);
  assert.match(commerce, /TEST pack/);
