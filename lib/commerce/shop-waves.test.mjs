...
test('Wave 2 Send plan never moves money', () => {
  ...
  assert.match(plan.copy, /Money does not move/);
  assert.doesNotMatch(plan.copy, /estimate not a transfer/i);
...
test('Legacy Wave assets remain packaged but do not mount on the current member page', () => {
  ...
  assert.doesNotMatch(html, /commerce-waves\.js/);
  assert.doesNotMatch(html, /commerce-waves\.css/);
});

test('collect card uses Nest Ompal and Ved Road, no hard comparison words', () => {
  assert.match(waves, /<h2>Collect at \$\{COLLECT\.name\}<\/h2>/);
  assert.match(waves, /Pick up your bag at \$\{COLLECT\.area\}\. No home delivery\./);
  assert.doesNotMatch(waves, /collect point|pickup window|doorstep|10-minute/);
  assert.match(build, /vendors\.html/);
  assert.match(build, /payout\.html/);
  assert.match(build, /save-desk\.html/);
});
