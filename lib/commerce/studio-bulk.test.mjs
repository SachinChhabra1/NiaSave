...
function rgbPng(path, width, height, color) {
  const py = spawnSync('python3', ['-c', `from PIL import Image; Image.new('RGB', (${width}, ${height}), tuple(${JSON.stringify(color)})).save(${JSON.stringify(path)})`], { encoding: 'utf8' });
...
test('photos from different source sizes normalize to identical 1200x900 WebP', () => {
  ...
  try {
    sources.forEach(([w, h, color], i) => {
      ...
      const img = spawnSync('python3', ['-c', `from PIL import Image; im=Image.open(${JSON.stringify(dest)}); print(im.size[0], im.size[1], im.format)`], { encoding: 'utf8' });
      assert.equal(img.status, 0, img.stderr);
      assert.equal(img.stdout.trim(), '1200 900 WEBP');
      sizes.add(img.stdout.trim());
    });
...
test('preview overlay replaces dummy Live cards with TEST studios and blocks reserve', async () => {
  ...
  try {
    ...
    assert.equal(quoted.body.error, 'test_studio_not_reservable');
  } finally {
    rmSync(dir, { recursive: true, force: true });
    clearTestStudios();
  }
...
test('bulk HTTP is preview-only and production is 403', async () => {
  ...
  try {
    ...
    assert.equal(blocked.body.error, 'preview_required');
  } finally {
