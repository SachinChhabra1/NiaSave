...
export function isTestLoginPath(path) {
  return path === '/test/member-login' || path === '/test/staff/login' || path === '/test/staff/prove' || path === '/test/identity';
}
...
export async function proveStaffLogin(env = process.env) {
  ...
  let headers = {};
  try {
    const token = issueStaffToken(result.staff);
    await registerStaffSession(token);
    headers = { 'set-cookie': staffPageCookie(token) };
  } catch {
    headers = {};
  }
  return {
