...
test('SMS identity is not used for the labelled TEST phone; real phones still hit the provider', async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (String(url).includes('/request')) return Response.json({challenge: 'provider-challenge'});
    if (String(url).includes('/verify')) return Response.json({ok: true});
    if (String(url).includes('/member')) return {status: 404, json: async () => ({error: 'member_not_registered'})};
  ...
  try {
    ...
    assert.equal(calls.some(entry => String(entry.url) === 'https://identity.invalid/request'), true);
...
test('GET /test/staff/login proves TEST desk password login without exposing a token', async () => {
  ...
  assert.equal(proof.headers?.['set-cookie'], undefined);
});
