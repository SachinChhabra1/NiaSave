/**
 * Live TEST member for one active unpaid Save order.
 ...
 * Finished rows (expired, cancelled, returned, missed, collected) do not block the next reserve.
 */
...
export const TEST_DESK = Object.freeze({
  ...
  login: 'admin@nia.one',
  flow: ['reserved', 'picked', 'packed', 'loaded', 'out_for_delivery', 'verify_payment', 'collect_at_stop', 'reconcile']
});

export function isTestMemberPath(path) {
  return path === '/test/session' || path === '/test/place-one' || /^\/test\/orders\/ord-[a-zA-Z0-9-]+$/.test(path) || /^\/test\/proof\/ord-[a-zA-Z0-9-]+$/.test(path);
}
...
/**
 * Finished TEST rows must not pin the one-active reserve.
 * In-flight fulfillment (reserved through return_pending, including a paid
 * handover that is not yet collected) still short-circuits.
 */
const TEST_ORDER_DONE = new Set(['cancelled', 'expired', 'returned', 'missed', 'collect_at_stop']);

export function testOrderBlocksFreshReserve(order) {
  return !TEST_ORDER_DONE.has(core.canonStatus(order?.status));
}

export function existingTestOrder(s) {
  return (s?.orders || []).find(o =>
    o.source === 'commerce' &&
    o.memberId === TEST_MEMBER.id &&
    o.test === true &&
    testOrderBlocksFreshReserve(o)
  ) || null;
}

/** Replay a still-active TEST reserve. A key that only points at a finished row gets a stable successor so reserve can create another. */
export function testMemberReserveKey(s, requestedKey) {
  let key = typeof requestedKey === 'string' && requestedKey ? requestedKey : TEST_ORDER_IDEMPOTENCY_KEY;
  if (!/^[a-zA-Z0-9_-]{16,100}$/.test(key)) return key;
  const seen = new Set();
  while (!seen.has(key)) {
    seen.add(key);
    const prior = s?.commerce?.requests?.[core.hash(TEST_MEMBER.id + ':' + key)];
    if (!prior?.orderId) return key;
    const order = (s?.orders || []).find(o => o.id === prior.orderId);
    if (order && testOrderBlocksFreshReserve(order)) return key;
    const basis = String(order?.id || prior.orderId);
    const next = `nia-test-after-${seen.size}-${basis}`.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 100);
    if (next.length < 16 || next === key) return key;
    key = next;
  }
  return key;
}
...
export function placeOneTestOrder(s, time, skus = SKUS, preview = false) {
  ...
  const reserved = core.reserve(s, actor, { ...intent, fingerprint: quoted.fingerprint }, testMemberReserveKey(s, TEST_ORDER_IDEMPOTENCY_KEY), skus, preview, time);
...
export function testOrderProof(s, order, req, env = process.env) {
  ...
  return {
    ...
    status: staffOrderView(order).status,
