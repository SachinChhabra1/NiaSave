...
test('unsigned POST /orders still requires a real member; guest TEST packs are orderable in the bag', async () => {
  const guest = await request('/catalogue');
  assert.equal(guest.status, 200);
  assert.equal(guest.body.account, null);
  const oil = guest.body.products.find(p => p.id === 'groundnut_oil');
  assert.equal(oil.pack, '1 L bottle');
  assert.equal(oil.test, true);
  assert.equal(guest.body.products.some(p => p.id === 'bathsoap_pick'), false);
  assert.equal(guest.body.products.some(p => p.id === 'essentials_pick'), false);
...
test('admin@nia.one Save desk can see the TEST order and walk reserve→pack→ready→UTR→close→recon', async () => {
  ...
  assert.equal(ready.body.status, 'out_for_delivery');
  ...
  const proof = await request(placed.body.readPath.replace('/api/commerce', ''));
  assert.equal(proof.body.status, 'collect_at_stop');
...
test('an expired TEST order does not stick; POST /orders and place-one can reserve a new one', async () => {
  const session = await request('/test/session');
  const cookie = session.headers['set-cookie'].split(';')[0];
  const saved = rows.get('test-member-prove');
  assert.ok(saved?.state_value?.orders?.length);
  const stuckIds = [];
  for (const order of saved.state_value.orders) {
    if (order.memberId !== TEST_MEMBER.id || order.test !== true || order.source !== 'commerce') continue;
    order.status = 'expired';
    order.payStatus = 'unpaid';
    order.expiresAt = '2026-09-19T06:47:51.316Z';
    stuckIds.push(order.id);
  }
  assert.ok(stuckIds.length >= 1);
  store.resetRuntimeCache();

  const intent = {locationId: 'S01', fulfillment: 'pickup', lines: [{id: 'groundnut_oil', qty: 1}]};
  const quote = await request('/quote', intent, {cookie});
  assert.equal(quote.status, 200);
  const placed = await request('/orders', {...intent, fingerprint: quote.body.fingerprint}, {
    cookie,
    headers: {'idempotency-key': TEST_ORDER_IDEMPOTENCY_KEY}
  });
  assert.equal(placed.status, 201);
  assert.equal(placed.body.status, 'reserved');
  assert.equal(placed.body.payStatus, 'unpaid');
  assert.equal(placed.body.memberId, TEST_MEMBER.id);
  assert.equal(placed.body.test, true);
  assert.equal(stuckIds.includes(placed.body.id), false);
  assert.equal(placed.body.lines[0].id, 'groundnut_oil');

  const again = await request('/orders', {...intent, fingerprint: quote.body.fingerprint}, {
    cookie,
    headers: {'idempotency-key': 'nia-test-post-order-after-expiry'}
  });
  assert.equal(again.status, 200);
  assert.equal(again.body.id, placed.body.id);
  assert.equal(again.body.status, 'reserved');

  const viaPlace = await request('/test/place-one');
  assert.equal(viaPlace.status, 200);
  assert.equal(viaPlace.body.order.id, placed.body.id);
  assert.equal(viaPlace.body.status, 'reserved');

  const cancelled = await request('/cancel', {orderId: placed.body.id}, {cookie});
  assert.equal(cancelled.status, 200);
  assert.equal(cancelled.body.status, 'cancelled');

  const quoteAfterCancel = await request('/quote', intent, {cookie});
  assert.equal(quoteAfterCancel.status, 200);
  const afterCancel = await request('/orders', {...intent, fingerprint: quoteAfterCancel.body.fingerprint}, {
    cookie,
    headers: {'idempotency-key': TEST_ORDER_IDEMPOTENCY_KEY}
  });
  assert.equal(afterCancel.status, 201);
  assert.equal(afterCancel.body.status, 'reserved');
  assert.notEqual(afterCancel.body.id, placed.body.id);
  assert.equal(stuckIds.includes(afterCancel.body.id), false);

  const unsigned = await request('/orders', {...intent, fingerprint: quoteAfterCancel.body.fingerprint});
  assert.equal(unsigned.status, 401);
  assert.equal(unsigned.body.error, 'sign_in_required');
});
