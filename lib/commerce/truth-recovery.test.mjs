...
test('LESS primary nav is Live Earn Shop Send', () => {
  const nav = lessNavCopy();
  assert.deepEqual([nav.live.brand, nav.earn.brand, nav.shop.brand, nav.send.brand], ['Live', 'Earn', 'Shop', 'Send']);
...
test('LESS primary nav stays Live Earn Shop Send', () => {
  assert.match(commerceSrc, /\['live','earn','shop','send'\]\.map/);
  ...
  assert.match(commerceSrc, /shopCatalogueMarkup/);
  assert.doesNotMatch(commerceSrc.slice(commerceSrc.indexOf('function showBuy('),commerceSrc.indexOf('const statusText =')),/Collect at Nia Nest Ompal/);
