...
import { COMMITMENTS_FROZEN, saveLaunchEnabled } from '../../commerce-capabilities.js';
...
export function frozenRoute(method, rawPath, env = process.env) {
  if (!COMMITMENTS_FROZEN) return false;
  const path = rawPath.replace(/^\/api(?=\/|$)/,'').replace(/\/+$/,'') || '/';
  if (method === 'POST' && ['/commerce/quote','/commerce/orders','/commerce/cancel'].includes(path)
      && saveLaunchEnabled(env)) return false;
...
export function saveBook(state) {
  const {commerce, memberSession, ...book} = state;
  const {sessions,accounts,memberPasswords,memberSetupGrants,testOtps,limits,centralNonces,tickets,issues,...rest} = commerce || {};
...
export function preserveFrozenBook(source, candidate) {
  ...
  next.commerce ||= {};
  if(candidate.memberSession!==undefined)next.memberSession=structuredClone(candidate.memberSession);
  for(const key of ['sessions','accounts','memberPasswords','memberSetupGrants','testOtps','limits','centralNonces','tickets','issues']) {
