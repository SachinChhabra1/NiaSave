...
test('authentication saves preserve raw frozen fields despite restore-time normalization',()=>{
 ...
 test('existing legacy authentication can change phone without changing the operational book',async()=>{
 const state=resetDummy();
 const before=structuredClone(state);
 const response=await request('/api/auth/verify','POST',{phone:'fixture-member-two'});
 assert.equal(response.status,200);
 assert.equal(response.body.memberId,'fixture-member-two');
 assert.equal(state.memberSession.phone,'fixture-member-two');
 assert.equal(saveBookUnchanged(before,state),true);
 const saved=preserveFrozenBook(before,state);
 assert.deepEqual(saved.memberSession,state.memberSession);
 assert.deepEqual(saved.orders,before.orders);
 assert.deepEqual(saved.beat,before.beat);
 const changed=structuredClone(state);changed.beat.opening.groundnut_oil=999;
 assert.equal(saveBookUnchanged(before,changed),false);
 });
