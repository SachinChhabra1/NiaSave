import {p0ReadOnly} from './p0-request-context.mjs';
...
export async function loadRuntimeState(stateKey, initialValue) {
  ...
  try {
    if (p0ReadOnly()) {
      const rows = await sql()`SELECT state_value, version FROM nia_runtime_state WHERE state_key = ${stateKey}`;
      if (!rows[0]) return {value: initialValue, version: 0, storage: "postgres"};
      return {value: asRuntimeValue(rows[0].state_value, initialValue), version: Number(rows[0].version), storage: "postgres"};
    }
    await ensureSchema();
...
export async function saveRuntimeState(stateKey, value, expectedVersion) {
  if (p0ReadOnly()) return {ok:false, storage:"postgres", error:"read_only_request"};
  stateKey = keyFor(stateKey);
  if (!hasDurableStore()) {
...
export async function storageStatus(stateKey) {
  ...
  try {
    if (!p0ReadOnly()) await ensureSchema();
    const rows = await sql()`
