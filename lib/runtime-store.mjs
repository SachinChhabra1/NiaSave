...
// M0 inspection/browse must never create a missing operational book.
export async function loadExistingRuntimeState(stateKey, initialValue) {
  if (!hasDurableStore()) return {value:initialValue,version:0,storage:'memory'};
  const rows = await sql()`SELECT state_value, version FROM nia_runtime_state WHERE state_key = ${keyFor(stateKey)}`;
  if (!rows[0]) return {value:null,version:0,storage:'postgres'};
  return {value:asRuntimeValue(rows[0].state_value,null),version:Number(rows[0].version),storage:'postgres'};
}
