...
const sikh=new Set(['ops','polo','pickup','recon','predict','hub','next','cash','source','inventory','ageing','po','dispatch','invoice','biker','vendors','payout','save-desk']);
...
const signIn='<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#F4F6FA"><title>Nia · Staff sign-in</title><link rel="stylesheet" href="/staff.css"><link rel="stylesheet" href="/nia-operator.css"></head><body><main class="staff-signin-page"><h1>Sign in to your desk</h1><p id="staff-entry-status" role="status">Use your Nia email and operator password.</p></main><script src="/staff.js"></script><script src="/staff-entry.js"></script></body></html>';
...
export async function staffPageAccess(request) {
  ...
  const saveInventory=name==='save-inventory';
  if(!living.has(name)&&!sikh.has(name)&&!shared.has(name)&&!dogra&&!saveInventory)return;
  ...
  const allowed=saveInventory?canAccessSaveStaff(staff):staff.role==='admin'||shared.has(name)||(living.has(name)&&staff.desks.some(d=>['living','studio','money'].includes(d)))||(sikh.has(name)&&staff.desks.some(d=>['studio','hub','money','pilot'].includes(d)))||(dogra&&staff.desks.some(d=>['studio','money','pilot'].includes(d)));
