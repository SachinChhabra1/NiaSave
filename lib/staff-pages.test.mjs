...
const {issueStaffToken,staffPageCookie,STAFF_PAGE_COOKIE}=await import('./staff-auth.mjs');
...
test('every deployed desk and clean alias challenges before serving a desk document',async()=>{
  for(const page of ['ops','polo','desk','2para','bison','bison-data','bison-clocks','bison-contracts','bison-collections','bison-nests','bison-studios','pickup','recon','predict','hub','next','cash','source','inventory','ageing','po','dispatch','invoice','biker','tanot/index']) {
    for(const suffix of ['','.html','/']) {
      const r=staffPageAccess(request('/'+page+suffix));
  ...
  const head=staffPageAccess(request('/ops.html','','HEAD'));assert.equal(head.status,401);assert.equal(await head.text(),'');
});
test('Ajay can open Living pages but cannot open Sikh or Dogra pages',()=>{
  const token=issueStaffToken(ajay);
  assert.equal(staffPageAccess(request('/bison-data.html',token)),undefined);
  assert.equal(staffPageAccess(request('/bison.html',token)),undefined);
  for(const path of ['/ops.html','/polo','/tanot','/tanot/index.html','/tanot/assets/bundle.js'])assert.equal(staffPageAccess(request(path,token)).status,403);
  assert.equal(staffPageAccess(request('/ops.html',issueStaffToken(admin))),undefined);
  assert.equal(staffPageAccess(request('/bison-data.html',token+'x')).status,401);
  assert.equal(staffPageAccess(request('/bison-data.html',issueStaffToken(ajay,Date.now()-13*3600000))).status,401);
});
test('login assets and application authentication routes stay available to obtain a credential',()=>{
  for(const path of ['/staff.js','/staff.css','/staff-entry.js','/v1/staff/login','/v1/staff/me','/api/bison/tower','/api/service/member','/commerce.html','/'])assert.equal(staffPageAccess(request(path)),undefined);
});
