...
test('every deployed desk and clean alias challenges before serving a desk document',async()=>{
  for(const page of ['ops','polo','desk','2para','bison','bison-data','bison-clocks','bison-contracts','bison-collections','bison-nests','bison-studios','pickup','recon','predict','hub','next','cash','source','inventory','ageing','po','dispatch','invoice','biker','vendors','payout','save-desk','save-inventory','tanot/index']) {
...
test('Save operator opt-in opens only the new Save page and retains all other desk denials',async()=>{
  const previous=process.env.NIASAVE_SAVE_OPERATOR_EMAILS;
  const token=issueStaffToken(ajay);await registerStaffSession(token);
  try{
    delete process.env.NIASAVE_SAVE_OPERATOR_EMAILS;
    assert.equal((await staffPageAccess(request('/save-inventory.html',token))).status,403);
    process.env.NIASAVE_SAVE_OPERATOR_EMAILS=' AJAY.MAHAWAR@NIA.ONE ';
    for(const path of ['/save-inventory','/save-inventory.html','/save-inventory/'])assert.equal(await staffPageAccess(request(path,token)),undefined);
    for(const path of ['/ops.html','/save-desk.html','/inventory.html','/tanot'])assert.equal((await staffPageAccess(request(path,token))).status,403);
    assert.equal((await staffPageAccess(request('/save-inventory.html',token+'x'))).status,401);
  }finally{if(previous===undefined)delete process.env.NIASAVE_SAVE_OPERATOR_EMAILS;else process.env.NIASAVE_SAVE_OPERATOR_EMAILS=previous;}
});
...
test('login assets and application authentication routes stay available to obtain a credential',async()=>{
  for(const path of ['/staff.js','/staff.css','/nia-operator.css','/save-desk-ui.css','/staff-entry.js','/v1/staff/login','/v1/staff/me','/api/bison/tower','/api/service/member','/commerce.html','/'])assert.equal(await staffPageAccess(request(path)),undefined);
});
