...
if(process.env.NIA_M1_EXPECT_REMOVED_HASH &&
   manifest.removedHash!==process.env.NIA_M1_EXPECT_REMOVED_HASH)
  throw new Error('discard_inventory_changed_no_success_claim');
...
if(Object.entries(post.manifest.counts).some(([,count])=>count!==0))
  throw new Error('discard_postcheck_failed');
